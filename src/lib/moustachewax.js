...
export default function () {
	...
	Handlebars.registerHelper("gurpslink", function (str, root, clrdmods = false) {
		...
		let template = document.createElement("textarea");
	...
	Handlebars.registerHelper("round", function (num) {
		...
		let temp = `${num}`
			...
			.replace(/^(-?\d+(?:\.\d+)*?) +.*/, "$1");
		// @ts-ignore
		return Number(`${Math.round(`${temp}e+2`)}e-2`);
	});
	...
	Handlebars.registerHelper("displayItemHover", function (obj) {
		return (
			...
			!!obj.img &&
			obj.img != "assets/svg/item-bag.svg"
		);
	});
...
`;
	...
	Handlebars.registerHelper("damageTerm", function (calc, options) {
		...
		let damageType =
			calc.damageType !== "dmg" && calc.damageType !== "injury" && calc.damageType !== "none"
				? calc.damageType
				: "";
