...
import { Skill, Advantage, Ranged, Note, Melee, Encumbrance, Equipment } from "../module/actor/actor-components.js";
import { digitsOnly } from "./jquery-helper.js";
import * as HitLocations from "../module/hitlocation/hitlocation.js";
import { i18n, sanitize } from "./utilities.js";
import * as settings from "../lib/miscellaneous-settings.js";

Hooks.once("init", async function () {
	game.settings.registerMenu(settings.SYSTEM_NAME, settings.SETTING_MOOK_DEFAULT_EDITOR, {
		name: i18n("GURPS.settingMookGenerator"),
		label: i18n("GURPS.settingLabelMookGenerator"),
		hint: i18n("GURPS.settingHintMookGenerator"),
...
export class NpcInput extends FormApplication {
	...
	activateListeners(html) {
		...
		const frame = html[0]?.querySelector(".npc-input");
		if (frame)
	...
	applyDrop(data) {
		if (data.actorid) {
			const actorData = game.actors.get(data.actorid).data;
			let element = getProperty(actorData, data.key);
	...
	async populate(a) {
		...
		att.ST.import = Number(m.st);
		att.DX.import = Number(m.dx);
		att.IQ.import = Number(m.iq);
		att.HT.import = Number(m.ht);
		att.WILL.import = Number(m.will);
		att.PER.import = Number(m.per);

		data.HP.max = Number(m.hp);
		data.HP.value = Number(m.hp);
		data.FP.max = Number(m.fp);
		data.FP.value = Number(m.fp);
