...
export function addColorWheelsToSettings() {
	$('#color-sheets input[type="checkbox"]').on("click", function () {
		...
		if ($(this).prop("checked")) {
			...
			$(`.${overrideColor}-hover-text .color`).css(
				"background-color",
				$(`.default-${overrideColor}-hover-text`).val()
			);
...
export function saveColorWheelsToSettings() {
	...
	SETTING_COLOR_ROLLABLE.forEach(function (rollableSheetColors) {
		...
		if (
			html.find(".gurps-sheet-colors").find(`.${rollableSheetColors}-hover-text .colorInput`).val() === undefined
		) {
			colorHoverText.push(`${SETTING_DEFAULT_COLOR_TEXT_HOVER}`);
		} else {
			colorHoverText.push(
				html.find(".gurps-sheet-colors").find(`.${rollableSheetColors}-hover-text .colorInput`).val()
			);
		}
	});
