...
export class CompositeDamageCalculator {
	...
	get resource() {
		...
		if (this._applyTo === "FP") return [this._defender.system.FP, "data.FP"];
		return [this._defender.system.HP, "data.HP"];
	}
...
class DamageCalculator {
	...
	get isKnockbackEligible() {
		...
		if ((this._parent.damageType === "cr" || this._parent.damageType === "kb") && this._basicDamage > 0)
			return true;
	...
	get isBluntTraumaInjury() {
		return (
			this.injury === 0 &&
			this._parent.isFlexibleArmor &&
			this._parent.useBluntTrauma &&
			this.effectiveBluntTrauma > 0
		);
	}
