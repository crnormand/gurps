...
export default class ApplyDamageDialog extends Application {
	...
	activateListeners(html) {
		...
		html.find("#override-dr input").on("change", ev =>
		...
		html.find("#adddamagemodifier").on("change", ev =>
			this._updateModelFromInputText($(ev.currentTarget), "damageModifier", t => t)
		);

		// ==== Tactical Rules ====
		// use armor divisor rules
		html.find("#tactical-armordivisor").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "useArmorDivisor")
		);

		// Armor divisor level
		html.find('select[name="tactical-armordivisor"]').on("change", ev =>
			this._updateModelFromSelect($(ev.currentTarget), "armorDivisor", parseFloat)
		);

		// Use blunt trauma rules
		html.find("#tactical-blunttrauma").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "useBluntTrauma")
		);

		// Use hit location wounding modifiers rules
		html.find("#tactical-locationmodifier").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "useLocationModifiers")
		);

		// ==== Other situations ====
		// is a ranged attack and at 1/2 damage or further range
		html.find("#specials-range12D").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "isRangedHalfDamage")
		);
		...
		html.find('select[name="hardened"]').on("change", ev =>
			this._updateModelFromSelect($(ev.currentTarget), "hardenedDRLevel", parseInt)
		);

		// Target has Injury Tolerance
		html.find("#injury-tolerance").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "isInjuryTolerance")
		);
		...
		html.find("#damage-reduction-field input").on("change", ev =>
		...
		html.find("#flexible-armor").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "isFlexibleArmor")
		);
		...
		html.find("#blunt-trauma-field input").on("change", ev => {
			...
			this._calculator.bluntTrauma =
				currentValue === "" || currentValue === this._calculator.calculatedBluntTrauma
					? null
		...
		html.find("#explosion-damage").click(ev =>
			this._updateModelFromCheckedElement($(ev.currentTarget), "isExplosion")
		);

		html.find("#explosion-yards").on("change", ev => {
			let currentValue = $(ev.currentTarget).val();
			this._calculator.hexesFromExplosion =
				currentValue === "" || currentValue === "0" ? 1 : parseInt(currentValue);
			this.updateUI();
		});
		...
		html.find("#shotgun-rof-multiplier").on("change", ev => {
			let currentValue = $(ev.currentTarget).val();
			this._calculator.shotgunRofMultiplier =
				currentValue === "" || currentValue === "0" ? 1 : parseInt(currentValue);
			this.updateUI();
		});
