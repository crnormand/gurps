...
class RangedAttackModel extends BaseAttack<RangedAttackSchema> {
  ...
  #prepareDisplayValues(): void {
    ...
    if (this.shots.reloadTime > 0) {
      this.shotsText += `(${this.shots.reloadTime.toString() + this.shots.realodTimeIsPerShot ? 'i' : ''}s`
    }
  }
...
const rangedAttackSchema = () => {
  return {
    ...
    shots: new fields.SchemaField({
      ...
      realodTimeIsPerShot: new fields.BooleanField({ required: true, nullable: false, initial: false }),
