...
import { handlePdf, SJGProductMappings } from "@module/pdf-refs.js";
import { GurpsActor } from "@actor/actor.js";
import { GurpsItem } from "@module/item.js";
import { GurpsItemSheet } from "@module/item-sheet.js";
import {
...
import GURPSTokenHUD from "@module/token-hud.js";
import GurpsJournalEntry from "@module/journal.js";
import TriggerHappySupport from "@module/effects/triggerhappy.js";
...
import GURPSConditionalInjury from "@module/injury/foundry/conditional-injury.js";
import { HitLocation } from "@module/hitlocation/hitlocation.js";
import GurpsActiveEffect from "@module/effects/active-effect.js";
import { StatusEffect } from "@module/effects/effects.js";
...
import GurpsActiveEffectConfig from "@module/effects/active-effect-config.js";
import * as GURPSSpeedProvider from "@module/speed-provider.js";
...
let GURPS: any = undefined;

if (!(globalThis as any).GURPS) {
	GURPS = {};
	(globalThis as any).GURPS = GURPS; // Make GURPS global!
	GURPS.DEBUG = true;
...
`;
	...
	// CONFIG.RollTable.resultIcon = 'systems/gurps/assets/single-die.webp'
	 ...
	 * @returns {Promise.<string|ArrayBuffer|null>}   A Promise which resolves to the loaded text data
	 */
	...
	async function findBestActionInChain({ action, actor, event, targets, originalOtf }) {
		...
		const calculations = await Promise.all(
			actions.map(a =>
				GURPS.actionFuncs[a.type]({ action: a, actor, event, targets, originalOtf, calcOnly: true })
			)
		);
	...
	async function applyModifierDesc(actor, desc) {
		...
		if (m && actor && !actor.isSelf) {
			...
			if (target.match(/^tr/i)) {
				await GURPS.ChatProcessors.startProcessingLines(
					`/setEventFlags true false false\\\\/${target} -${delta}`
				); // Make the tracker command quiet
				return null;
			}
		}
	...
	GURPS.setInitiativeFormula = function(/** @type {boolean} */ broadcast) {
		let formula = /** @type {string} */ game.settings.get(
			Settings.SYSTEM_NAME,
			Settings.SETTING_INITIATIVE_FORMULA
		);
		if (!formula) {
	...
	Hooks.once("init", async function() {
		...
		let src =
			game.i18n.lang == "pt_br" ? "systems/gurps/assets/gurps4e-pt_br.webp" : "systems/gurps/assets/gurps4e.webp";
		...
		{
			let img = new Image();
			img.src = "systems/gurps/assets/blood-splatter-clipart-small.webp";
		...
		{
			let img = new Image();
			img.src = "systems/gurps/assets/all-the-things-transparent.webp";
		...
		Hooks.on("createRollTable", async function(entity, options, userId) {
			await entity.update({ img: "systems/gurps/assets/single-die.webp" });
			entity.data.img = "systems/gurps/assets/single-die.webp";
		});
		...
		Hooks.on("renderSidebarTab", async (app, html) => {
			...
			if (app.options.id === "compendium") {
				let button = $(
					`<button class="import-items"><i class="fas fa-file-import"></i>${game.i18n.localize("GURPS.itemImport")} </button>`);
...
<div style='font-size:70%'>
  ...
  <div style='text-align: center;'>
    ...
    <div><a href="https://ko-fi.com/crnormand"><img height="24" src="systems/gurps/assets/SupportMe_stroke@2x.webp"></a></div>
  </div>
</div>`,
			...
			if (game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_SHOW_CHANGELOG)) {
				...
				game.settings.set(
					Settings.SYSTEM_NAME,
					Settings.SETTING_CHANGELOG_VERSION,
					GURPS.currentVersion.toString()
				);
			}
		...
		Hooks.on("hotbarDrop", async (bar, data, slot) => {
			...
			if (oldmacro && oldmacro.getFlag("gurps", "drag-drop-otf")) {
				...
				new Dialog({
					...
					content: `Merge both macros into this:<br><br><mark>${oldmacro.data.command
						.split("\n")
						.join("<br>")}<br>${cmd
							.split("\n")
		...
		Object.values(CONFIG.Actor.sheetClasses.character).forEach(e => {
			if (e.id.toString().startsWith(Settings.SYSTEM_NAME) && e.id != "gurps.GurpsActorSheet")
