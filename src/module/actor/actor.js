...
export class GurpsActor extends Actor {
	...
	async postImport() {
		...
		let orig = /** @type {GurpsItem[]} */ (
			this.items.contents.slice().sort((a, b) => b.name.localeCompare(a.name))
	...
	calculateDerivedValues() {
		...
		this.system.conditions.maneuver = maneuver ? maneuver.data.flags.gurps.name : "undefined";
	...
	_initializeStartingValues() {
		...
		recurselist(data.skills, (e, k, d) => {
			// @ts-ignore
			if (e.import) e.level = parseInt(Number(e.import));
		});
					...
					if (link.action) {
						...
						recurselist(data.skills, (e, k, d) => {
							...
							if (link.action.type == "attribute") {
								...
								if (e.relativelevel?.toUpperCase().startsWith(link.action.attrkey))
									e.level += pi(link.action.mod);
							}
	...
	_getMoveAdjustedForManeuver(move, threshold) {
		...
		if (foundry.utils.getProperty(this.system, PROPERTY_MOVEOVERRIDE_MANEUVER)) {
			let value = foundry.utils.getProperty(this.system, PROPERTY_MOVEOVERRIDE_MANEUVER);
	...
	_getMoveAdjustedForPosture(move, threshold) {
		...
		if (foundry.utils.getProperty(this.system, PROPERTY_MOVEOVERRIDE_POSTURE)) {
			let value = foundry.utils.getProperty(this.system, PROPERTY_MOVEOVERRIDE_POSTURE);
	...
	_calculateRangedRanges() {
		...
		let st = Number(this.system.attributes.ST.value);
	...
	async update(data, context) {
		if (game.settings.get(settings.SYSTEM_NAME, settings.SETTING_AUTOMATIC_ONETHIRD)) {
			if (data.hasOwnProperty("data.HP.value")) {
				let flag = data["data.HP.value"] < this.system.HP.max / 3;
			...
			if (data.hasOwnProperty("data.FP.value")) {
				let flag = data["data.FP.value"] < this.system.FP.max / 3;
							...
							callback: html => {
								...
								if (!files.length) {
									...
									GURPS.readTextFromFile(file).then(text =>
										this.importFromGCSv1(text, file.name, file.path)
									);
								}
							},
						},
	...
	getPortraitPath() {
		if (game.settings.get(settings.SYSTEM_NAME, settings.SETTING_PORTRAIT_PATH) == "global")
	...
	importEq(i, p, carried) {
		...
		e.weight =
			(
				parseFloat(i.calc?.extended_weight) / (i.type == "equipment_container" ? 1 : i.quantity || 1)
			).toString() || "0";
		...
		for (let i of hls.locations) {
			...
			for (let [key, value] of Object.entries(i.calc?.dr))
				if (key != "all") {
					...
					if (!l.split) l.split = {};
					l.split[damtype] = Number(l.import) + value;
				}
		...
		if (table) {
			Object.values(locations).forEach(it => {
				...
				if (entry) {
					...
					if (!it.roll || it.roll.length === 0 || it.roll === HitLocations.HitLocation.DEFAULT)
						it.roll = entry.roll;
				}
			});
		}
	...
	async importFromGCSv2(json, importname, importpath, suppressMessage = false, GCAVersion, GCSVersion) {
		...
		try {
			...
			console.log(
				`Done importing (${Math.round(performance.now() - starttime)}ms.)  You can inspect the character data below: )`);
			console.log(this);
			return true;
		} catch (err) {
			console.log(err.stack);
	...
	async importFromGCSv1(xml, importname, importpath, suppressMessage = false) {
		...
		let c;
		var ra; // The character json, release attributes
		...
		try {
			...
			if (!suppressMessage) ui.notifications?.info(i18n_f("GURPS.importSuccessful", { name: nm }));
			console.log(
				`Done importing (${Math.round(performance.now() - starttime)
	...
	// When reading data, use "this.system.skills", however, when updating, use "data.skills".
	/**
	 * @param {{ [key: string]: any }} json
	 * @param {boolean} isFoundryGCS
	 */
	...
	// When reading data, use "this.system.spells", however, when updating, use "data.spells".
	/**
	 * @param {{ [key: string]: any }} json
	 * @param {boolean} isFoundryGCS
	 */
	...
	async handleEquipmentDrop(dragData) {
		...
		if (this.isOwner && srcActor.isOwner) {
			...
			if (eqt.count < 2) {
				...
				if (destKey) {
					// Already have some
					let destEqt = getProperty(this.system, destKey);
	...
	async updateItem(item) {
		...
		let oldeqt;
		if (oldkey) oldeqt = getProperty(this.system, oldkey);
	...
	async _addNewItemEquipment(itemData, targetkey) {
		...
		if (existing) {
			...
			let eqt = getProperty(this.system, existing);
		...
		if (targetkey == null)
			if (_data.carried) {
				...
				let index = 0;
				let list = getProperty(this.system, targetkey);
	...
	async deleteEquipment(path, depth = 0) {
		let eqt = getProperty(this.system, path);
		if (!eqt) return eqt;
	...
	async _removeItemElement(itemid, key) {
		...
		while (found) {
			found = false;
			let list = getProperty(this.system, key);
	...
	async moveEquipment(srckey, targetkey, shiftkey) {
		...
		let object = getProperty(this.system, srckey);
			...
			buttons: {
				...
				two: {
					...
					callback: async () => {
						...
						let targ = getProperty(this.system, targetkey);
	...
	async toggleExpand(path, expandOnly = false) {
		let obj = getProperty(this.system, path);
	...
	async _splitEquipment(srckey, targetkey) {
		let srceqt = getProperty(this.system, srckey);
	...
	async _checkForMerging(srckey, targetkey) {
		let srceqt = getProperty(this.system, srckey);
		let desteqt = getProperty(this.system, targetkey);
		if (
	...
	async updateEqtCount(eqtkey, count) {
		...
		await this.update(update);
		let eqt = getProperty(this.system, eqtkey);
	...
	async updateParentOf(srckey, updatePuuid = true) {
		...
		let parent = getProperty(this.system, sp);
		if (parent) {
			// Data protection
			...
			if (updatePuuid) {
				...
				if (paths.length >= 6) {
					...
					puuid = getProperty(this.system, sp).uuid;
				}
				await this.internalUpdate({ [`${srckey}.parentuuid`]: puuid });
				let eqt = getProperty(this.system, srckey);
				if (eqt.itemid) {
					let item = this.items.get(eqt.itemid);
					if (item)
