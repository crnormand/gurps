...
class StaticCharacterGURPS extends BaseActorGURPS {
	...
	// 			...(this.flags[SYSTEM_NAME][ActorFlags.TargetModifiers]),
	// 			{
	...
	// 	let orig: StaticItemGURPS[] = (this.items.contents as StaticItemGURPS[]).sort((a, b) => b.name?.localeCompare(a.name ?? "") ?? 0)
	...
	// 		}
	// 	}
	// 	// for (const item of good) await this.addItemData(item.system)
	...
	// 		Static.recurseList(this.system.languages, (e, k, d) => {
	...
	// 		)
	// 			if (
	// 				isNaN(data.attributes[attr as StaticAttributeName].import)
	// 			) data.attributes[attr as StaticAttributeName].value = 0
	// 	}
	// 	Static.recurseList(data.skills, (e, _k, _d) => {
	// 		if (!!e.import) e.level = Number(+e.import)
	// 	})
