...
async function migrate(): Promise<MigrationReport | void> {
  ...
  const actors = game.actors.contents.filter(
    // @ts-expect-error: Using legacy type which is no longer defined in the type configuration
  ...
  if (length > 0) {
    ...
    // NOTE: We don't need to run migrateActorSource against the documents, because is is already called
