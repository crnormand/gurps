...
// @deprecated
// export function bindEquipmentCrudActions(html: HTMLElement, actor: Actor.Implementation): void {
...
//
//       await openItemSheetIfFoundryItem(actor, equipmentData)
//     })
//   })
//
...
export function bindTrackerActions(html: HTMLElement, actor: Actor.Implementation): void {
  ...
  //     }
  //
  //     const confirmed = await tracker.deleteDialog()
  //
  ...
  resetButtons.forEach(button => {
    button.addEventListener('click', async (event: MouseEvent) => {
      ...
      if (!isHTMLElement(target)) return
      const uuid = target.closest<HTMLElement>('[data-uuid]')?.dataset.uuid ?? ''
      const tracker = actor.getEmbeddedDocument('ResourceTracker', uuid) as TrackerInstance | undefined
