...
export default class StatusChatProcessor extends ChatProcessor {
	 ...
	 */
	list() {
		let html = `<table style='font-size: smaller;'><tr><th>${i18n("GURPS.ID")}</th><th>${i18n(
			"GURPS.name"
		)}</th></tr>`;
		/** @type {EffectData[]} */
		let sortedEffects = [];
	...
	getSelfTokens() {
		...
		let msg =
			list && list.length === 0 ? i18n("GURPS.chatNoOwnedTokenFound") : i18n("GURPS.chatMultipleOwnedFound");
		ui.notifications.warn(msg);
		return null;
	}
	...
	async toggleTokenEffect(token, effect, actionText) {
		if (effect) {
			...
			this.prnt(
				`${i18n(actionText)} <i>${effect.id}:'${i18n(effect.label)}'</i> ${i18n("GURPS.for")} ${
					actor.displayname
				}`
			);
		}
