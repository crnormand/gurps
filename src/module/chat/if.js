...
import { evaluateBlocks, IfBlockParser } from './if-block-parser.ts'
...
export class IfChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    const blocks = IfBlockParser.parse(line)

    const result = await evaluateBlocks(blocks, this.resolveCondition.bind(this))
   ...
   * @returns
   */
  async resolveCondition(condition, line) {
  ...
  async performOtfAction(action, line) {
    this.priv(line)
    this.send()
    let event = this.msgs().event

    event.chatmsgData = this.msgs().data
    let pass = await GURPS.modules.Otf.performAction(action, GURPS.LastActor, event)
