...
export async function findBestActionInChain({
  ...
  for (const action of actions) {
    ...
    if (func.constructor.name === 'AsyncFunction') {
      resolvedActions.push(await func({ action: action, actor, event, targets, originalOtf, calcOnly: true }))
    } else {
      resolvedActions.push(
        func({ action: action, actor, event, targets, originalOtf, calcOnly: true }) as
