...
class OtfParser {
  ...
  parse_(input: InputArgs): ParserResult | null {
    if (this.regex && this.regex.test(input.str)) {
      const match = input.str.match(this.regex)
  ...
  parse(_match: RegExpMatchArray, _input: InputArgs): ParserResult | null {
    return null
  }
}
...
class ModifierParser extends OtfParser {
  ...
  override parse(match: RegExpMatchArray, args: InputArgs): any | null {
    const mod = match.groups?.signednum
    const sign = mod![0]
...
class MarginModParser extends OtfParser {
  ...
  override parse(match: RegExpMatchArray, args: InputArgs): ParserResult | null {
    ...
    const desc = mod + ' ' + match.groups!.modtext.trim()
    let spantext = desc
    let def = null
...
class AdvLevelModParser extends OtfParser {
  ...
  override parse(match: RegExpMatchArray, args: InputArgs): ParserResult | null {
    const sign = match.groups!.sign
    const modtext = match.groups!.modtext.trim()
    const mod = `${sign}A:${modtext}`
    const temp = `${sign}${match.groups!.tag}:${modtext}`
...
class AttributeParser extends OtfParser {
  ...
  override parse(match: RegExpMatchArray, args: InputArgs): ParserResult | null {
    ...
    let target
    const targetMatch = attr.match(/^(?<attrname>[a-zA-Z ]+)(?<target>\d+)/)
    ...
    if (!path) {
      const targetMatch = attr.split(' ') // but most others do not
    ...
    const desc = opt ? opt.groups!.attr.trim() : ''
    ...
    if (match.groups!.remainder) {
      ...
      if (def?.action?.type == OtfActionType.skillSpell || def?.action?.type == OtfActionType.attribute)
...
class SkillSpellParser extends OtfParser {
  ...
  parseSkill(input: string): DeepPartial<Action> | null {
    ...
    function match(re: RegExp): RegExpExecArray | null {
      const matches = re.exec(first.slice(i))

      if (!matches || matches.index !== 0) return null
      i += matches[0].length
    ...
    if (matches) {
      ...
      const attribute = parselink(result.floatingLabel)
    ...
    if (remainder) {
      const def = parselink(remainder.trim())
...
class AttackDamageParser extends OtfParser {
  ...
  override parse(match: RegExpMatchArray, args: InputArgs): ParserResult | null {
    ...
    if (modifier) {
      ...
      const mrg = comment.match(/^([+-])@margin(.*)/i)
    ...
    const isMelee = !!args.str.match(/^[AMDPB]/i)
...
export function gmspan(
  ...
  let span = `<span class='gga-app glinkmod'${otfaction}>${str}`

  if (clrdmods) {
    if (plus) span = `<span class='gga-app glinkmodplus'${otfaction}>${str}`
    else span = `<span class='gga-app glinkmodminus'${otfaction}>${str}`
  }
...
export function gspan(
  ...
  let span = "<span class='gga-app gurpslink'"
