...
export function validateDice(dice: string, _options = {}): boolean | void {
  ...
  if (!match) {
    console.warn(`Dice string "${normalizedDice}" does not match the expected pattern.`)

    return false
  }

  if (match.groups!.sides && parseInt(match.groups!.sides) <= 0) {
    console.warn(`Dice string "${normalizedDice}" has invalid sides value: ${match.groups!.sides}`)

    return false
  }

  if (match.groups!.number && parseInt(match.groups!.number) <= 0) {
    console.warn(`Dice string "${normalizedDice}" has invalid number of dice: ${match.groups!.number}`)
