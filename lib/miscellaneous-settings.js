...
export function initializeSettings() {
  Hooks.once('init', async function () {
    ...
    game.settings.register(SYSTEM_NAME, SETTING_CHECK_EQUIPPED, {
      ...
      hint: "If checked, ONLY 'Equipped' items will be included in the weight calculation.   If unchecked, all items in the Equipment list are included in the weight calculation.   Uncheck, and the players can use the 'Equipped' flag to indicate they have dropped the item.",
      scope: 'world',
      config: true,
      type: Boolean,
      default: false,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_SHOW_USER_CREATED, {
      name: 'Actor: Display User-created Flag',
    ...
    game.settings.register(SYSTEM_NAME, SETTING_SHOW_FOUNDRY_CREATED, {
      name: 'Actor: Display Foundry Item Flag',
    ...
    game.settings.register(SYSTEM_NAME, SETTING_SHIFT_CLICK_BLIND, {
      name: 'Enable Blind rolls for Players',
      hint: 'If not checked, holding down the SHIFT key when you are making a roll will make a Self roll (only you can see it, no matter who you are). If checked, when players hold down the SHIFT key, it will make it a Blind Roll instead (GMs still get a Self roll)',
      scope: 'world',
      config: true,
      type: Boolean,
      default: false,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_ONLY_GMS_OPEN_ADD, {
      ...
      hint: 'If checked, only GMs can view/use the Apply Damage Dialog. Players who drag and drop damage will just receive a message.',
      scope: 'world',
      config: true,
      type: Boolean,
      default: true,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_SHOW_THE_MATH, {
      ...
      hint: "If checked, the 'SHOW THE MATH' section of the damage chat message will be expanded by default. If unchecked, you can click on the 'SHOW THE MATH' title to expand it.  NOTE: If this is the last message in the chat log, it may expand below the bottom of the log, and you might need to scroll down to see it.",
      scope: 'world',
      config: true,
      type: Boolean,
      default: true,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_AUTOMATIC_ONETHIRD, {
      ...
      hint: 'If checked, Reeling (1/2 move/dodge) and Tired (1/2 move/dodge/ST) will automatically be turned on when the character drops below 1/3 HP and 1/3 FP.',
      scope: 'world',
      config: true,
      type: Boolean,
      default: false,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_PLAYER_CHAT_PRIVATE, {
      ...
      hint: "If checked, most of the chat commands (e.g. /hp, /fp, /qty, etc.) will display as a whisper to the player (instead of an Out Of Character message).   If you trust your players, and don't like to see the chat fill up with their chat commands, turn this on",
      scope: 'world',
      config: true,
      type: Boolean,
      default: false,
