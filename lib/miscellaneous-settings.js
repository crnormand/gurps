...
export function initializeSettings() {
  Hooks.once('init', async function () {
    ...
    game.settings.register(SYSTEM_NAME, SETTING_MANEUVER_VISIBILITY, {
      name: i18n('GURPS.settingManeuverVisibility', 'Maneuver Icon Visibility'),
      hint: i18n('GURPS.settingHintManeuverVisibility', "Determines who can see a token's maneuver icon."),
      scope: 'world',
      config: true,
      type: String,
      choices: {
        NoOne: i18n('GURPS.settingManeuverNoOne', 'No One'),
        GMAndOwner: i18n('GURPS.settingManeuverGMOnly', 'GM and Owner Only'),
        Everyone: i18n('GURPS.settingManeuverEveryone', 'Everyone'),
      },
      default: 'NoOne',
      onChange: value => Migration.migrateToManeuvers(value !== 'NoOne'),
    })

    game.settings.register(SYSTEM_NAME, SETTING_MANEUVER_DETAIL, {
      name: i18n('GURPS.settingManeuverDetail', 'Maneuver Visibility Details'),
      hint: i18n(
        'GURPS.settingHintManeuverDetail',
        `Determines what maneuver is visible to players other than the GM or Owner, if any. 
'Full Detail': E.g., show "All-out Attack (Determined)" and "Feint" exactly as shown. 
'No Feint': As 'Full Detail' except Feint shows as "Attack". 
'General': As 'Full, No Feint' except subtype is not shown -- e.g., All-out Attack (Strong) shows as "All-out Attack". 
(GM and Owner always see Full Detail.)`
      ),
      scope: 'world',
      config: true,
      type: String,
      choices: {
        Full: i18n('GURPS.settingManeuverDetailFull', 'Full Detail'),
        NoFeint: i18n('GURPS.settingManeuverDetailNoFeint', 'No Feint'),
        General: i18n('GURPS.settingManeuverDetailGeneral', 'General'),
      },
      default: 'General',
      onChange: value => Migration.changeMigrationDetail(value),
    })
