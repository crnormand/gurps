...
export function initializeSettings() {
  Hooks.once('init', async function () {
    ...
    game.settings.register(SYSTEM_NAME, SETTING_PHYSICAL_DICE, {
      name: i18n('GURPS.settingUsePhysicalDice', 'Use Physical Dice'),
      hint: i18n(
        'GURPS.settingHintUsePhysicalDice',
        'If enabled, TRUSTED players will be prompted for all die roll values. This allows players to roll physical dice and enter the results into Foundry.'
      ),
      scope: 'client',
      config: true,
      type: Boolean,
    ...
    game.settings.register(SYSTEM_NAME, SETTING_REMOVE_UNEQUIPPED, {
      ...
      onChange: value => {
        for (const actor of game.actors.contents) {
          ...
          if (actor.sheet) actor.sheet.render()
        }
      }
    })
