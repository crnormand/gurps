...
import { isArray, isEmpty, getComparison, getOperation, i18n, i18n_f, zeroFill, recurselist } from './utilities.js'
...
export default function () {
  ...
  Handlebars.registerHelper('gurpslink', function (str, root, clrdmods = false) {
    ...
    return new Handlebars.SafeString(GURPS.gurpslink(str, root == true || clrdmods == true))
  })
  ...
  Handlebars.registerHelper('gurpslinkbr', function (str, root, clrdmods = false) {
    let actor = root?.data?.root?.actor
    if (!actor) actor = root?.actor
    return GURPS.gurpslink(str, root == true || clrdmods == true).replace(/\n/g, '<br>')
  })
  ...
  Handlebars.registerHelper('attackflatlist', function (context) {
    let data = {}
    flatlist(context, 0, '', data, false, game.settings.get(settings.SYSTEM_NAME, settings.SETTING_REMOVE_UNEQUIPPED) ? this.actor : null)
    return data
  })
  ...
  const flatlist = function (context, level, parentkey, data, isCollapsed, actorToCheckEquipment) {
    ...
    for (let key in context) {
      ...
      let display = true
      if (actorToCheckEquipment) { // if we have been given an actor, then check to see if the melee or ranged item is equipped in the inventory
        recurselist(actorToCheckEquipment.data.data.equipment.carried, e => { // check
          if (item.name.startsWith(e.name) && !e.equipped) display = false
        })
        recurselist(actorToCheckEquipment.data.data.equipment.other, e => {
  ...
  Handlebars.registerHelper('pdflink', function (link) {
    ...
    return !!txt
      ? txt
          .split(',')
          .map((/** @type {string} */ l) => GURPS.gurpslink(`[PDF:${l}]`))
          .join(', ')
      : ''
  })
  ...
  Handlebars.registerHelper('pdflinkext', function (obj) {
    ...
    return txt
          ...
          .map((/** @type {string} */ l) => {
            ...
            else return GURPS.gurpslink(`[PDF:${l}]`)
          })
          .join(', ')
  })
  ...
  Handlebars.registerHelper('displayNumber', function (num) {
    if (num != null) {
      ...
      if (arguments[1] != false && num.toString()[0] !== '+') return `+${num}`
      return num.toString()
   ...
   */
  Handlebars.registerHelper('tracker', function (data, trackerName) {
    if (!!data && !!data.additionalresources?.tracker) {
      // find the tracker with name 'Control Points'
  ...
  /**
   * Unlike the 'threshold-of' method above, this returns the *last* threshold whose
   * condition matches.
   */
  ...
  Handlebars.registerHelper('thresholdBreakpoints', function (tracker) {
    /**
     * @type {{ breakpoint: number; label: any; comparison: string; }[]}
     */
    let results = []
    tracker.thresholds.forEach(
      (/** @type {{ operator: string; value: number; comparison: string; condition: any; }} */ threshold) => {
        let op = getOperation(threshold.operator)
  ...
  /**
   * Douglas Cole's Fantastic Dungeon Grappling is a special case. The first threshold
   * breakpoint is rounded up; the rest are rounded down.
   */
  Handlebars.registerHelper('controlBreakpoints', function (tracker) {
     ...
     */
    let results = []
    tracker.thresholds.forEach((threshold, index) => {
      let op = getOperation(threshold.operator)
      let temp = op(tracker.max, threshold.value)
      let isLessThan = threshold.comparison === '≤' || threshold.comparison === '<'
  ...
  Handlebars.registerHelper('displayItemHover', function (obj) {
    return (
      game.settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_FOUNDRY_CREATED) &&
      !!obj.img &&
      obj.img != 'icons/svg/item-bag.svg'
    )
  })
...
`
  ...
  Handlebars.registerHelper('damageTerm', function (calc, options) {
    ...
    let damageType = calc.damageType !== 'dmg' && calc.damageType !== 'none' ? calc.damageType : ''
