...
export default function () {
  ...
  const flatlist = function (context, level, parentkey, data, isCollapsed, actorToCheckEquipment) {
    ...
    for (let key in context) {
      ...
      if (actorToCheckEquipment) {
        ...
        let checked = false
        recurselist(actorToCheckEquipment.getGurpsActorData().equipment.carried, e => {
          // check
        ...
        if (!checked)
          recurselist(actorToCheckEquipment.getGurpsActorData().equipment.other, e => {
  ...
  Handlebars.registerHelper('inCombat', function (data) {
    if (data.actor && !!game.combats.active) {
      return game.combats.active.combatants.contents.map(it => it.actor?.id).filter(e => !!e).includes(data?.actor?.id)
    }
    return false
  })
