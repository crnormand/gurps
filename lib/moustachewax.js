...
export default function () {
  ...
  Handlebars.registerHelper('flatlist', function (context) {
    let data = {}
    flatlist(context, 0, '', data)
    return data
  })

  const flatlist = function (context, level, parentkey, data) {
    ...
    for (let key in context) {
      ...
      for (let propertyKey in item) {
        // copy everything except 'contains' and 'collapsed' and 'indent'
      ...
      newItem['isCollapsed'] = parentkey.endsWith('.collapsed.')
      newItem['isContained'] = parentkey.endsWith('.contains.')
      data[newKey] = newItem

      flatlist(item?.contains, level + 1, newKey + '.contains.', data)
      flatlist(item?.collapsed, level + 1, newKey + '.collapsed.', data)
    }
  }
...
`
  ...
  const templates = [
    ...
    'systems/gurps/templates/actor/sections/other-equipment.hbs',
