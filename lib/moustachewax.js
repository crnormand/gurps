...
import { extractOtfs } from '../module/otf/extract-otfs.js'
...
export default function () {
  ...
  Handlebars.registerHelper('gurpslink', function (str, root, clrdmods = true) {
  ...
  Handlebars.registerHelper('gurpslinkbr', function (str, root, clrdmods = true) {
    let actor = root?.data?.root?.actor
    if (!actor) actor = root?.actor
  ...
  Handlebars.registerHelper('extractOtfs', function (...args) {
    return extractOtfs(args.slice(0, -1)) // the last argument is Handlebars options, so ignore it
  })
  ...
  Handlebars.registerHelper('listAllManeuvers', function (current) {
    return Maneuvers.getAllInPlayData(typeof current === 'string' ? current : null)
  })
