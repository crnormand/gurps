...
import { _game, _GURPS } from '../module/global-references.js'
...
export default function () {
  ...
  Handlebars.registerHelper('gurpslink', function (str, root, clrdmods = false) {
    ...
    return new Handlebars.SafeString(_GURPS().gurpslink(str, root == true || clrdmods == true))
  })
  ...
  Handlebars.registerHelper('gurpslinkbr', function (str, root, clrdmods = false) {
    ...
    if (!actor) actor = root?.actor
    return _GURPS()
      .gurpslink(str, root == true || clrdmods == true)
      .replace(/\n/g, '<br>')
  })
  ...
  Handlebars.registerHelper('listeqt', function (context, options) {
    ...
    let ans = _GURPS().listeqtrecurse(context, options, 0, data)
    return ans
  })
  ...
  Handlebars.registerHelper('listattack', function (src, key, options) {
    ...
    let context = src[key]
    let ans = _GURPS().listeqtrecurse(context, options, 0, data, '', src)
    return ans
  })
  ...
  Handlebars.registerHelper('showTheMath', function () {
    return _game().settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_THE_MATH) ? 'checked' : ''
  })
  ...
  Handlebars.registerHelper('pdflink', function (link) {
    ...
    return !!txt
      ? txt
          .split(',')
          .map((/** @type {string} */ l) => _GURPS().gurpslink(`[PDF:${l}]`))
          .join(', ')
      : ''
  })
    ...
    return !!txt
      ? txt
          ...
          .map((/** @type {string} */ l) => {
            ...
            else return _GURPS().gurpslink(`[PDF:${l}]`)
          })
          .join(', ')
      : ''
  })
  ...
  Handlebars.registerHelper('isUserCreated', function (obj) {
    return _game().settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_USER_CREATED) && !!obj.save
  })

  Handlebars.registerHelper('isFoundryItem', function (obj) {
    return _game().settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_FOUNDRY_CREATED) && !!obj.itemid
  })

  Handlebars.registerHelper('ignoreImportQty', function (obj) {
    return _game().settings.get(settings.SYSTEM_NAME, settings.SETTING_ignoreImportQty) && !!obj.ignoreImportQty
  })

  Handlebars.registerHelper('displayItemHover', function (obj) {
    return (
      _game().settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_FOUNDRY_CREATED) &&
      !!obj.img &&
