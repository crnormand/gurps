...
export default function () {
  ...
  Handlebars.registerHelper('localizeWithFallback', function (value, fallback) {
    let result = game.i18n.localize(value)
    return value === result ? fallback : result
  })
  ...
  Handlebars.registerHelper('threshold-of', function (thresholds, max, value) {
    ...
    thresholds.some(function (threshold, index) {
      let op = getOperation(threshold)
      let comparison = getComparison(threshold)
  ...
  Handlebars.registerHelper('thresholdBreakpoints', function (tracker) {
    ...
    tracker.thresholds.forEach(threshold => {
      let op = getOperation(threshold)
      results.push({
        breakpoint: Math.floor(op(tracker.max, threshold.value)),
...
function getOperation(threshold) {
  let op = function (a, b) {
    return a + b
  }
...
function getComparison(threshold) {
