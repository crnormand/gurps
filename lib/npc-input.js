...
they vanish instantly if wounded to .1\HP, and can also be
...
//    ts.modifiedon = dt;
  ...
  parseStatBlock(txt) {
	...
	storeAttrib(attrib, value) {
		value = value.replace(/[;.,]$/,"");		// strip off any ending ;.,
		attrib.toLowerCase().split("/").forEach(a => this.mook[a] = value);	// handles cases like "Parry/Block: 9"
	}
	
	gatherAttackLines(attblk, currentline) {// read lines looking for (\d+) and any lines following that do NOT have (\d+)
		var nextline;
	...
	parseSkills() {
		...
		let skills = "";
		while(line.match(/\w+-\d+/)) {
			skills += " " + line.trim();
			line = this.nextToken("\n", false, true);
		}
		if (!skills) throw "No skills matching pattern 'skill-lvl' found";
		this.mook.skills = "";
		var l;
		while (!!skills) {
	...
	parseTraits() {
		let trblk = this.nextToken("Skills:", false);
		if (!trblk) throw "Looking for Traits block, unable to find 'Skills:' after Attack block";
		if (trblk[trblk.length-1] == ".") trblk = trblk.substring(0, trblk.length-1);
		let traits = "";
		trblk.replace(/\n/g," ").split(";").forEach(t => {
			t = t.trim();
			let m = t.match(/(.*)\((\d+)\)/);
	...
	parseAttacks(last) {
      ...
      { regex: "[Rr]anged,?", var: "" },
		...
		this.mook.melee = "";
		this.mook.ranged = "";
		let attblk = this.nextToken("Traits:", false);
		if (!attblk) throw "Unable to find 'Traits:' after Attribute block";
		attblk = last + " " + attblk;
		...
		[attblk, line, nextline] = this.gatherAttackLines(attblk, line);	// collect up any more lines.
		...
		while (!!line) {
			...
			if (!name) throw `No name detected for attack "${save}"`;
			name = name.trim();
			[line, lvl] = this.nextTokenPrim(line, ")", "):");	
			if (!lvl || Number.isNaN(lvl)) throw `No attack level or level is not a number "${save}"`;
			[line, dmg] = this.nextTokenPrim(line, ".", ",", true);		// find up to . or , or end of string	
			let note = "";
			[dmg, note] = this.mapDmg(save, dmg);
			if (!!note) note = "\n:" + note;
			...
			if (!!result?.groups?.reach) {		// If it has Reach, it is definitely melee
				...
				line = line.replace(/[Rr]each [^.]+/, "").trim();
			...
			[attblk, line, nextline] = this.gatherAttackLines(attblk, nextline);	// collect up any more lines.
		}
	}
	
	mapDmg(line, dmg) {
		if (!dmg) throw `No damage formula for attack "${line}"`;
		let p = GURPS.parseDmg(dmg.trim().toLowerCase());
		if (p == dmg) throw `Damage "${dmg}" does not parse correctly for "${line}"`;
		let a = p.split("~");
		let roll = a[0] + "d" + a[1] + a[2] + a[3] + a[4];
		let types = a[5].trim().split(" ")
		let m = GURPS.damageTypeMap[types[0]]
		if (!m) throw `Unrecognized damage type "${type}" for "${line}"`
	...
	checkForNameNotes() {
		...
		if (line.includes(":") || line.match(oldregex)) return pushToken(line + "\n");
		...
		if ((line.match(/ /g)||[]).length < 3) 	
			...
			notes += "\n" + line;
		line = this.nextToken("\n");
		while (!line.includes(":") && !line.match(oldregex)) {
			notes += "\n" + line;
			line = this.nextToken("\n");
		}
		this.pushToken(line + "\n");
		this.mook.notes = notes;
	}
	
	pushToken(t) {
		this.statblk = t + this.statblk;
	}
