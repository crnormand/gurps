...
import {
  ...
  Skill,
  Spell,
} from '../module/actor/actor-components.js'
import { GurpsActor } from '../module/actor/actor.js'
import * as HitLocations from '../module/hitlocation/hitlocation.js'
import getUserInput from '../module/utilities/get-user-input.js'
import { translate } from './i18n.js'
import { digitsAndDecimalOnly, digitsAndSignOnly, digitsOnly, signedIntegersOnly } from './jquery-helper.js'
...
export class NpcInput extends FormApplication {
  ...
  activateListeners(html) {
    ...
    html
      .find('.ms-attr-pair input:not([data-key="speed"]):not([data-key="damage"]):not([data-key="sm"])')
    ...
    html.find('.ms-attr-pair input[data-key="sm"]').inputFilter(value => digitsAndSignOnly.test(value))

    html.find('input[type=text]:not([data-key="damage"]):not([data-key="sm"])').on('change paste keyup', event => {
    ...
    html.find('input[type=text][data-key="sm"]').on('paste keyup', event => {
      const element = event.currentTarget
      const key = element.dataset.key
      if (key) {
        // If the input is valid, update the mook. If not, don't update but also don't set testing to false since they might just be in the middle of typing.
        if (signedIntegersOnly.test(element.value)) {
          this.mook[key] = element.value
          this.setTesting()
        }
      }
    ...
    html.find('input[type=text][data-key="sm"]').on('change', event => {
      const element = event.currentTarget
      const key = element.dataset.key
      if (key) {
        if (signedIntegersOnly.test(element.value)) {
          ...
          ui.notifications.warn(game.i18n.localize('GURPS.mookGeneratorInvalidSMInput'))
          // Reset the input to the last valid value.
          element.value = this.mook[key] || '+0'
        }
      }
    })
  ...
  async createMook(ev) {
    ...
    if (this.testing) {
      ...
      let actor = await GurpsActor.create(data, { renderSheet: false })
      await this.populate(actor)
      await actor.setFlag('core', 'sheetClass', 'gurps.GurpsActorNpcModernSheet')
  ...
  async populate(actor) {
    ...
    att.ST.import = +mook.st
    att.DX.import = +mook.dx
    att.IQ.import = +mook.iq
    att.HT.import = +mook.ht
    att.WILL.import = +mook.will
    att.PER.import = +mook.per
    ...
    let ns = {}
    let nt = new Note(mook.notes.trim())
    GURPS.put(ns, { ...nt })
    ...
    e.key = 'enc0'
    e.weight = 0
    e.move = parseInt(mook.move)
    e.dodge = parseInt(mook.dodge)
    GURPS.put(es, { ...e })

    let melee = {}
    mook.a_melee.forEach(me => GURPS.put(melee, me))

    let ranged = {}
    mook.a_ranged.forEach(r => GURPS.put(ranged, r))

    let ts = {}
    ts.title = mook.title
    let dt = new Date().toString().split(' ').splice(1, 3).join(' ')
    ts.createdon = dt
    if (!!mook.sm) ts.sizemod = mook.sm[0] === '-' ? mook.sm : mook.sm[0] === '+' ? '' : '+' + mook.sm
    ts.appearance = mook.desc
    ts.height = mook.height
    ts.weight = mook.weight
    ts.age = mook.age

    let skills = {}
    mook.a_skills.forEach(s => GURPS.put(skills, s))

    let spells = {}
    mook.a_spells.forEach(s => GURPS.put(spells, s))

    let ads = {}
    mook.a_traits.forEach(a => GURPS.put(ads, a))

    let eqt = {}
    mook.a_equipment.forEach(e => GURPS.put(eqt, e))

    let dmg = mook.damage || mook.dmg || ''
  ...
  checkSpells() {
    ...
    this.prep(m.spells, ';').forEach(e => {
      ...
      if (isNaN(v)) return (txt += `\n${ERR} "${v}" is not a number`)
      arr.push({ ...new Spell(n, v) })
    })

    m.spells = txt.substr(1)
    m.a_spells = arr
    return txt.includes(ERR)
  }
