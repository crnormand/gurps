...
export class NpcInput extends FormApplication {
  ...
  isValidDamageInput(value) {
    // A GURPS die roll: one or more digits, followed by "d", with an optional modifier of the format +/- followed by digits, and an optional multiplier of the format "x" or "*" followed by digits.
    const gurpsDieRollPattern = /\d+d(?:[+-–]\d+)?/
    const gurpsDamagePattern = new RegExp(`^${gurpsDieRollPattern.source}(?:/${gurpsDieRollPattern.source})?$`)

    return gurpsDamagePattern.test(value)
  }
