...
export class NpcInput extends Application {
  ...
  async createMook(ev) {
		ev.preventDefault();
	  if (ev.shiftKey) this.parseStatBlock(EX[parseInt(this.mook.name)], parseInt(this.mook.name));
		else
    if (this.check()) {
  ...
  parseStatBlock(txt, i) {
	...
	parseAttacks() {
      ...
      { regex: " [Rr]ange ([0-9\/]+) *,?", var: "range" },
		...
		let attblk = this.nextToken("Traits:", "Advantages/Disadvantages:");		// Look for either as start of ads/disads
		if (!attblk) return this.mook.melee= COMMENT_CHAR + "No attacks found";
	...
	isAttribute(a) {
		...
		return [ "basic", "speed", "move", "dodge", "sm", "parry", "block", "dr", "damage", "bl", "fright", "check" ].includes(a.toLowerCase());
	}
	...
	parseAttrs() {
		...
		while (!done) {
			...
			any = false;
			[line, attr] = this.nextTokenPrim(line);
