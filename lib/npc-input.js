...
export class NpcInput extends Application {
  ...
  async createMook(ev) {
		...
		if (this.testing) {
			let err = this.check();
	    if (!!err)  
				ui.notifications.warn("Unable to create Mook: " + err);
			else
  ...
  checkMelee() {
    ...
    this.prep(m.melee).forEach(e => { 
      ...
      let parse = e.replace(/(.*) ?\((\d+)\) (\d+)d6?([-+]\d+)?([xX\*]\d+)?(\([.\d]+\))?(!)? ?(\w+)(.*)$/g, "$1~$2~$3~$4~$5~$6~$7~$8~$9");
			if (e != parse) {
     		parse = parse.split("~");
      	me = new Melee(parse[0].trim(), parse[1], parse[2] + "d" + parse[3] + parse[4] + parse[5] + parse[6] + " " + parse[7]);
				remain = parse[8].trim();
			} else {
				parse = e.replace(/(.*) ?\((\d+)\) "([^"]+)" ?(.*)$/g, "$1~$2~$3~$4");
  ...
  parseStatBlock(txt) {
	...
	parseSkills() {
		...
		let goUntil = this.peek("Spells");

		if (!line) return;
		line = this.cleanLine(line);
		let skills = "";
		while(!!line && (line.match(/[^-]+-\d+/) || (!!goUntil && !line.match(/^Spells/)))) {
			skills += "\n" + this.cleanLine(line).replace(/ ?\[-?\d+\],?\.? ?/g, " ");	
			line = this.nextToken("\n", false, true);
		}
		if (!skills) this.appendToNotes("?? No skills matching pattern 'skill-lvl' found");
		skills = this.cleanLine(skills);
		let delim = ";";   
		if (skills.includes(delim)) 
			skills = skills.replace(/\n/g, " ");	// If has delims, then remove newlines
		else
			delim = "\n";	// No ";", so assume one per line
		var l;
		while (!!skills) {
				[skills, l] = this.nextTokenPrim(skills, delim, false, true);	 	// Start it off reading the first line
	...
	parseSpells() {
		this.trim();
		if (!this.peek("Spells")) return;
		this.nextToken("Spells");
		let goUntil = this.peek("Equipment");
		let line = this.nextToken("\n", false, true);
		if (!line) return;
		line = this.cleanLine(line);
		let spells = "";
		while(!!line && (line.match(/[^-]+-\d+/) || (!!goUntil && !line.startsWith("Equipment")))) {
			spells += "\n" + this.cleanLine(line).replace(/ ?\[-?\d+\],?\.? ?/g, " ");
			line = this.nextToken("\n", false, true);
		}
		if (!spells) this.appendToNotes("?? No spells matching pattern 'spell-lvl' found");
		spells = this.cleanLine(spells);
		let delim = ";";   
		if (spells.includes(delim)) 
			spells = spells.replace(/\n/g, " ");	// If has delims, then remove newlines
		else
			delim = "\n";	// No ";", so assume one per line
		var l;
		while (!!spells) {
				[spells, l] = this.nextTokenPrim(spells, delim, false, true);	 	// Start it off reading the first line
				...
				if (m) {
					...
					this.mook.spells += "\n" + COMMENT_CHAR + "Unknown spell pattern " + l;
		}
		if (!!line) this.pushToken(line);
	}
	...
	parseTraits() {
		...
		let traits = "";
		if (trblk.includes(";")) 
			trblk = trblk.replace(/\n/g," ");		// if it contains ";", then remove newlines
		else {		// no ";", maybe using commas?
		...
		this.prep(trblk,";").forEach(t => {
			...
			let m = t.match(/(.*)\((\d+)\)/);
			if (!!m) 
				traits += `\n[CR: ${m[2]} ${m[1].trim()}]`;		// Convert CR roll into OtF
			else {
				if (t != "and")
					traits += "\n" + t;
			}
	...
	checkForNameNotes() {
	  this.trim();
		...
		let first = true;
		while (!!line && !line.match(curregex) && !line.match(oldregex) && !line.match(forumregex)) {		// Assume we haven't started the stat block yet
			// if the first line has 2 or fewer spaces, assume that it is a name.   Just guessing here
			if (first && (line.match(/ /g)||[]).length < 3) 	
				this.mook.name = line;
			else
	...
	storeAttrib(attrib, value) {
		...
		attrib.toLowerCase().split("/").forEach(a => this.mook[a] = val);	// handles cases like "Parry/Block: 9"
	...
	parseAttrs() {
		...
		do {
			...
			while(this.isAttribute(attr)) {	// Something we recognize
	  ...
	  this.pushToken(saved);		// We didn't use this line, so put it back
		let a = unknowns.map(a => a.join(" ")).join(" ").trim(); // collapse all unknowns into a string
		if (!!a) {	// we parsed some things that did not work.
