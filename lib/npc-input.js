...
Bite (15): 1d.1(2) cutting. Reach C.
...
export class NpcInput extends Application {
  ...
  async createMook(ev) {
		ev.preventDefault();
	  if (ev.shiftKey) this.parseStatBlock(EX[parseInt(this.mook.name)]);
		else
    if (this.check()) {
  ...
  parseStatBlock(txt) {
	...
	parseAttacks(last) {
      ...
      { regex: "[Rr]anged,?", var: "" },
		...
		if (!attblk) return this.mook.melee= ":No attacks found";
		attblk = last + " " + attblk;
		...
		while (!!line) {
			...
			if (!name)  return this.mook.melee= `:No attacks found, "${save}" `;
			name = name.trim();
	...
	mapDmg(line, dmg) {
		...
		if (!m) return [ "", `Unrecognized damage type "${type}" for "${line}"` ];
	...
	isAttribute(a) {
		if (a.endsWith(":") || !!GURPS.attributepaths[a]) return true;
		return [ "speed", "move", "dodge", "sm", "parry", "block", "dr", "damage", "bl", "basic" ].includes(a.toLowerCase());
	}
	
	getAttrib(t) {
		let a = t.replace(/[^A-Za-z]/g, "");
		// Special case is attributes are listed as "basic speed".
		return a.toLowerCase() == "basic" ? "" : a;
	}
