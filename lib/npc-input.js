...
export class NpcInput extends Application {
  ...
  parseStatBlock(txt) {
	...
	parseTraits() {
		...
		if (!trblk) return this.appendToNotes(ERR + " Looking for Traits block, unable to find 'Skills:' after Attack block");
		trblk = this.cleanLine(trblk);
		let traits = "";
		this.parseLines(trblk.replace(/\n/g," "),";").forEach(t => {
			t = t.trim().replace(/ ?\[-?\d+\],?.? ?/, " ");		// Remove points costs [20], [20].
			t = t.replace(/\( *(\d+) *or less *\)/g,"($1)");
			let m = t.match(/(.*)\((\d+)\)/);
			if (!!m) 
	...
	parseAttacks(oldformat = false) {
      ...
      { regex: " ?[Rr]ange ([0-9\/]+) *,?", var: "range" },
		...
		while (!!line) {
			...
			if (!name) {
				this.mook.melee += `${COMMENT_CHAR}No attacks found, "${save}" `;
			...
			if (!lvl || isNaN(lvl)) {
				this.mook.melee += `${COMMENT_CHAR}No attack level or level is not a number "${save}"`;
				[attblk, line, nextline] = this.gatherAttackLines(attblk, nextline, oldformat);	// collect up any more lines.
				continue;
			}
	...
	checkForNameNotes() {
	  this.trim();
		...
		let oldregex = /ST.*\d+.*DX.*\d+.*IQ.*\d.*HT.*\d+/
		
		let first = true;
		while (!line.match(curregex) && !line.match(oldregex)) {		// Assume we haven't started the stat block yet
	...
	parseAttrs() {
		...
		do {
			...
			while(this.isAttribute(attr)) {
				let goodattr = this.getAttrib(attr);
				if (!!goodattr) {
					...
					any = true;
				} else unknowns.slice(-1)[0].push(attr);
				if (!line) {
					line = this.nextline();
					any = false;
					unknowns.push([]);
	  ...
	  this.pushToken(line);		// We didn't use this line, so put it back
		let a = unknowns.map(a => a.join(" ")).join(" ");
