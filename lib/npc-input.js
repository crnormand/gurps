...
export class NpcInput extends Application {
  ...
  parseStatBlock(txt) {
	...
	parseSpells() {
		...
		var l;
		while (!!spells) {
				[spells, l] = this.nextTokenPrim(spells, ";", false, true);	 	// Start it off reading the first line
				...
				if (m) {
					...
					this.mook.spells += "\n" + COMMENT_CHAR + "Unknown skspellsill pattern '" + l + "'";
		}
		if (!!line) this.pushToken(line);
	}
	...
	checkForNameNotes() {
	  this.trim();
		...
		let first = true;
		while (!!line && !line.match(curregex) && !line.match(oldregex)) {		// Assume we haven't started the stat block yet
