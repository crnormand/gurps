...
export const DAMAGE_REGEX =
  /^(?<accum>\+)?(?<roll>\d+)(?<D>[dD]\d*)?(?<adds>[\–\-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*)(?<costs>\*([Cc]osts|[Pp]er)? \d+ ?[\w\(\) ]+)?$/
...
export const DERIVED_DAMAGE_REGEX =
  /^(?<accum>\+)?(?<att>SW|Sw|sw|THR|Thr|thr)()(?<adds>[\–\-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*)(?<costs>\*([Cc]osts|[Pp]er)? \d+ ?[\w\(\) ]+])?$/ // have fake 2nd element so indexes match
