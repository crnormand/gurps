...
export const DAMAGE_REGEX =
  /^(?<accum>\+)?(?<roll>\d+)(?<D>[dD]\d*)?(?<adds>[\–\-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*([Cc]osts|[Pp]er)? \d+ ?[\w\(\) ]+)?(?<follow>,.*)?$/
...
export const DERIVED_DAMAGE_REGEX =
  /^(?<accum>\+)?(?<att>SW|Sw|sw|THR|Thr|thr)()(?<adds>[\–\-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*([Cc]osts|[Pp]er)? \d+ ?[\w\(\) ]+])?(?<follow>,.*)?$/ // have fake 2nd element so indexes match
...
export function parseForRollOrDamage(str, overridetxt) {
  ...
  if (!!a) {
    ...
    var next
    if (a.groups.follow) next = parseForRollOrDamage(a.groups.follow.substring(1).trim()).action // remove ',')
