...
export function parselink(str, htmldesc, clrdmods = false) {
  ...
  parse = str.replace(/^[AD]:([^ "+-]+\*?) ?([-+]\d+)? ?(.*)/g, '$1~$2~$3')
  if (parse == str) {
  ...
  if (parse != str) {
    ...
    if (!!n) {
      ...
      let action = {
        orig: str,
        type: str.startsWith('A') ? 'attack' : 'attackdamage',
        name: n,
        mod: a[1],
        desc: moddesc,
        blindroll: blindroll,
      ...
      return {
        text: gspan(overridetxt, spantext, action, brtxt + '<b>' + (str.startsWith('A') ? 'A' : 'D')  + ':</b>', comment),
        action: action,
      }
    }
  }
...
function gmspan(overridetxt, str, action, plus, clrdmods) {
  if (!!overridetxt) str = overridetxt
