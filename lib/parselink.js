...
export function parselink(str, htmldesc, clrdmods = false) {
  ...
  if (!!m) {
    ...
    if (!!path) {
      ...
      } else { // since I couldn't parse the margin with the initial regex, lets check for it in the description
        let mrg = desc.match(/^([+-])@margin(.*)/)
        if (!!mrg) {
          desc = 'margin ' + mrg[2].trim()
          let sign = mrg[1]
          modifier = (GURPS.lastTargetedRoll?.margin || 0) * (sign == '+' ? 1 : -1)
          spantext += ' ' + sign + '@' + desc
        } else spantext += ' ' + desc
      }
      let def = null
  ...
  if (parse != str) {
    ...
    if (!!name) {
      ...
      if (!!modifierText) {
        ...
        let mrg = moddesc.match(/^([+-])@margin(.*)/)
        if (!!mrg) {
          moddesc = 'margin ' + mrg[2].trim()
          let sign = mrg[1]
          modifierText = (GURPS.lastTargetedRoll?.margin || 0) * (sign == '+' ? 1 : -1)
          spantext += ' ' + sign + '@' + moddesc
        }
      }
