...
export const DAMAGE_REGEX =
  /^(?<roll>\d+)(?<D>[dD]\d*)?(?<adds>[-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*)(?<costs>\*[Cc]osts? \d+ ?[\w\(\) ]+)?$/
...
export const DERIVED_DAMAGE_REGEX =
  /^(?<att>SW|Sw|sw|THR|Thr|thr)()(?<adds>[-+]@?\w+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*)(?<costs>\*[Cc]osts? \d+ ?[\w\(\) ]+])?$/ // have fake 2nd element so indexes match
