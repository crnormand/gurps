import { DamageTermParser } from '../module/damage/damage-parser.js'
...
export function parseForRollOrDamage(str, overridetxt) {
  ...
  let baseAction = undefined

  const action = _buildAction(baseAction, str.split(','))
...
function _buildAction(action, parts) {
  if (!parts || parts.length === 0) return undefined

  const s = parts.shift().trim() // remove the first element of the array of parts
  const parser = new DamageTermParser(s) // parse the damage term to validate it and extract components

  if (!parser.output) return undefined

  if (parser.output.derivedRoll) {
    action = {
      orig: s.trim(),
      type: 'deriveddamage',
      displayformula: `${parser.rollString}${parser.divisorString}`,
      damagetype: parser.output.type,
      extdamagetype: parser.output.extendedType,
      costs: parser.cost,
      hitlocation: parser.output.hitLocation,
      accumulate: parser.output.accumulator,
      derivedformula: parser.output.derivedRoll,

      formula: `${parser.formulaString}`,
      next: _buildAction(action, parts), // recursively build the next action
    }
  } else {
    action = {
      orig: s.trim(),
      type: 'damage',
      formula: `${parser.formulaString}`,
      displayformula: `${parser.formulaString}`,
      damagetype: parser.output.type,
      extdamagetype: parser.output.extendedType,
      costs: parser.cost,
      hitlocation: parser.output.hitLocation,
