...
import { _GURPS } from '../module/global-references.js'
...
export const DAMAGE_REGEX =
  /^(?<roll>\d+)(?<D>[dD]\d*)?(?<adds>[-+]\d+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<type>[^\*]*)(\*[Cc]osts? \d+ ?[\w\(\)]+)?$/
...
export const DERIVED_DAMAGE_REGEX =
  /^(?<att>SW|Sw|sw|THR|Thr|thr)()(?<adds>[-+]\d+)?(?<mult>[×xX\*]\d+)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<type>[^\*]*)(\*[Cc]osts? \d+ ?[\w\(\)]+])?$/ // have fake 2nd element so indexes match
...
export function parselink(str, htmldesc, clrdmods = false) {
  ...
  if (!!m) {
    ...
    // @ts-ignore
    let path = _GURPS().PARSELINK_MAPPINGS[attrkey] // fright check has a space
    if (!path) {
      ...
      path = _GURPS().PARSELINK_MAPPINGS[attrkey]
      m[4] = tmp.join(' ')
    }
    if (!!path) {
