...
export function parselink(str, htmldesc, clrdmods = false) {
  ...
  let overridetxt
  let m = str.match(/^"(?<overridetext>[^"]*)"(?<text>.*)/)
  if (m) {
    overridetxt = m.groups?.overridetext
    str = m.groups?.text.trim()
  } else {
    m = str.match(/^'(?<overridetext>[^']*)'(?<text>.*)/)
    if (m) {
      overridetxt = m.groups?.overridetext
      str = m.groups?.text.trim()
    }
  ...
  var sourceId
  m = str.match(/^@(?<actorid>[^@]+)@(?<text>.*)/)
  if (m) {
    sourceId = m.groups.actorid
    str = m.groups.text.trim()
  }
  ...
  m = tmp.match(/^(?<signednum>[\+-]\d+)(?<modtext>[^&]*)(?<othermods>&.*)?/)
  if (m) {
    let mod = m.groups.signednum
    let sign = mod[0]
    let desc = m.groups.modtext.trim()
    ...
    if (!!m.groups.othermods) {
      def = parselink(m.groups.othermods.slice(1).trim()) // remove the leading &
  ...
  m = str.match(/^(?<sign>[+-])@margin *(?<modtext>[^&]*)(?<othermods>&.*)?/i)
  if (m) {
    let sign = m.groups.sign
    let mod = sign + '@margin'
    let desc = mod + ' ' + m.groups.modtext.trim()
    let spantext = desc
    let def = null
  ...
  // if [@margin] [then]
  m = str.match(/^@(?<keyword>margin|isCritSuccess|IsCritFailure) *(?<expression>[=<>]+ *[+-]?[\d\.]+)?$/i)
  if (!!m) {
    let action = {
      type: 'iftest',
      orig: str,
      name: m.groups.keyword,
      equation: m.groups.expression,
    }
    return {
      text: str,
      action: action,
  ...
  m = str.match(/^(?<link>JournalEntry|JournalEntryPage|Actor|RollTable|Item)\[(?<id>[\.\w]+)\](?<overridetext>{.*})/)
  if (!!m) {
    let action = {
      type: 'dragdrop',
      orig: str,
    ...
    return {
      text: gspan(m.groups.overridetext, str, action),
      action: action,
    }
  }
  ...
  if (!!m) {
    ...
    if (!path) {
      ...
      m.groups.desc = tmp.join(' ')
    }
    if (!!path) {
      // TODO Understand this -- what options does this provide?
      let opt =
        m.groups.desc.trim().match(/(?<attr>[^\?]*)(\? *"(?<truetext>[^"]*)")?( *[,:] *"(?<falsetext>[^"]*)")?/) || [] // desc (searching for true/false options)
      let desc = opt.groups.attr.trim()
      let spantext = attr
      if (!!target) spantext += target
      let melee = ''
      if (!!m.groups.object) {
        melee = m.groups.object.slice(1).trim()
        spantext += ':' + melee
      }
      let modifier = m.groups.mod
      if (!!modifier) {
      ...
      let def = null
      if (!!m.groups.remainder) {
        def = parselink(m.groups.remainder.slice(1).trim())
      ...
      return {
        text: gspan(overridetxt, spantext, action, blindRollText),
        action: action,
      }
    }
  }
  ...
  if (two === 'CR' && str.length > 2 && str[2] === ':') {
    ...
    return {
      text: gspan(overridetxt, str, action, blindRollText),
      action: action,
    }
  }
  ...
  if (parse !== str) {
    ...
    if (!!name) {
      ...
      let prefix = blindRollText + '<b>S:</b>'
      if (isSpell) prefix = blindRollText + '<b>Sp:</b>'
      if (isSkill) prefix = blindRollText + '<b>Sk:</b>'
      let action = {
        orig: str,
        type: 'skill-spell',
        isSpellOnly: isSpell,
  ...
  if (parse != str) {
    ...
    if (!!n) {
      ...
      return {
        text: gspan(overridetxt, spantext, action, blindRollText + '<b>' + str[0].toUpperCase() + ':</b>', comment),
        action: action,
      }
    }
  }
