...
const _advLevelMod = {
  /**
    ...
    - A:example &mod
      ...
      - othermods: &mod
    */
  regex: /^(?<sign>[+-])[Aa]:(?<modtext>"[^"]+"|'[^']+'|[^ &]+) *(?<othermods>&.*)?/,
  ...
  parse: (match, args) => {
    ...
    let desc = match.input.replace(mod, '').trim()
    let spantext = `${mod} ${desc}`
    let action = {
      orig: match.input,
      spantext: spantext,
...
const _attribute = {
   ...
   *  • object = ":" alpha { alpha | " " | "*" } -- A string that starts with a letter, and contains only letters, spaces, and "*".
   *  • attr-name = alpha { alpha | " " } alpha -- A string that starts and ends with a letter, and contains only letters and spaces.
   *  • mod = [ "+" | "-" ] ( positive-integer | "@margin" )
   *  • desc = ( text - "|" ) -- Any string that does not contain the '|' character.
   */
  regex:
    /^(?<attrname>[A-Za-z][A-Za-z ]*[A-Za-z][0-9]*)(?<object>:[A-Za-z \*]+)? ?(?<mod>[+-](\d+|@margin))? ?(?<desc>[^\|]*)(?<remainder>\|.*)?$/,
