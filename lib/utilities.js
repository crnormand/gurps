...
export function isNiceDiceEnabled() {
  ...
  let niceDice = false
  try {
    niceDice = _game().settings.get('dice-so-nice', 'settings').enabled
  } catch {}
  return niceDice
}
...
export function getAllActorsInActiveScene() {
  ...
  let activeScene = _game().scenes?.active
  if (!!activeScene) {
    let tokens = activeScene.data.tokens
    let actorIds = tokens.map(it => it.actorId)
    actorIds.forEach(id => targets.push(_game().actors.get(id)))
  }
  return targets
}
...
export function i18n(value, fallback) {
  let result = _game().i18n.localize(value)
...
export function i18n_f(value, data, fallback) {
  let template = _game().i18n.has(value) ? value : fallback
  let result = _game().i18n.format(template, data)
  if (!!fallback) return value === result ? fallback : result
  return result
}
...
export function locateToken(identifier) {
  ...
  let tokens = _canvas().tokens?.placeables
  if (!!tokens) {
    // try token IDs first
