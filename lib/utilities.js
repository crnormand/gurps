...
/**
 * Flattens a nested object structure into a single array.
 * Items are considered containers if they have a `contains` property.
 * @param {Object} list - The object containing items to flatten
 * @returns {any[]} A flat array of all items (including nested ones)
 */
 ...
 * @returns {Record<string, any>} - an Object with properties indexed by zero-padded strings
 */
...
export function sanitize(str) {
  ...
  str = str.replace(/(x|&#215;|&#xD7;|&times;)/g, 'x') // we need to convert the multiplication symbol to ✕ for easier parsing
...
/**
 * Useful to display derived dice expressions like 'sw+2' or 'thr-1' as GURPS dice expressions.
 * @param {*} value the string value to convert.
 * @param { swing: string, thrust: string } system an object containing 'swing' and 'thrust' properties.
 * @returns
 */
export const displayAsDice = (value, system) => {
  ...
  if (!!system && system.thrust && system.swing && lowerValue.match(/^(sw|thr)/)) {
    ...
    // Here we need to check if both formula and df contains +add (like +1). If so, we need to sum the adds.
    const damageMatch = deriveddamage.match(/(?<dice>\d+)d(?<add>[+-]\d+)?/)
    const dice = damageMatch?.groups.dice || '0'
    const dfAdd = damageMatch?.groups.add || 0

    const formulaAdd = matches.groups.modifier || 0
