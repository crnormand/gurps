 ...
 * @param {name: string, mode: string|undefined|null} item
 * @returns {string}
 */
export function quotedAttackName(item) {
  const quoteCharacter = item.name.includes(DOUBLE_QUOTE) ? SINGLE_QUOTE : DOUBLE_QUOTE
  let n = !!item.mode ? item.name + ' (' + item.mode + ')' : item.name
  return quoteCharacter + n + quoteCharacter
}
