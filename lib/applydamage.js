...
export default class ApplyDamageDialog extends Application {
  ...
  updateUI(html) {
    ...
    this._isVulnerableCheckbox(html).prop('checked', this.isVulnerable)
  ...
  async _handleEffectButtonClick(ev) {
    ...
    if (object.type === 'shock') {
      message = await this._renderTemplate('effect-chat-shock.html',
        {
    ...
    if (object.type === 'majorwound') {
      message = await this._renderTemplate('effect-chat-majorwound.html',
        {
          name: this.actor.data.name,
    ...
    if (object.type === 'headvitalshit') {
      message = await this._renderTemplate('effect-chat-headvitalshit.html',
        {
          name: this.actor.data.name,
    ...
    if (object.type === 'knockback') {
      message = await this._renderTemplate('effect-chat-knockback.html',
        {
          name: this.actor.data.name,
          yards: object.amount,
  ...
  insertArmorDivisor(html, element) {
    this._renderTemplate(`result-armor-divisor.html`, {
      ...
      armorDivisor: this._calculator.armorDivisor,
  ...
  resolveInjury(injury, type, publicly, results = null) {
    ...
    this._renderTemplate('results-chat.html', data).then(html => {
...
export class DamageCalculator {
  constructor(defender, damageData) {
    ...
    if (Object.keys(this.defaultWoundModifiers).includes(damageData.damageType))
  ...
  get defaultWoundModifiers() {
    let filterModifiers = Object.keys(GURPS.woundModifiers).reduce(function (r, e) {
  ...
  //
  // Effective DR = DR adjusted for armor divisor, if any.
  //
  ...
  // 
  // Calculated Blunt Trauma = 1/10 of the Basic Damage* for the appropriate damage types; 1/5 of
  //    Basic Damage if damage type is 'cr' (Crushing).
  //
  // * - this might should be Effective Damage, not Basic.
  //
