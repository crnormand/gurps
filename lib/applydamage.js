...
export default class ApplyDamageDialog extends Application {
  constructor(actor, damageData, options = {}) {
    ...
    this.applyDivisor = game.settings.get(settings.SYSTEM_NAME, settings.SETTING_APPLY_DIVISOR)
  ...
  activateListeners(html) {
    ...
    html.find('.pdflink').click(this._onClickPdf);

    // If this isSimpleDialog, hide the advanced panel and resize the window.
    let applyDamageAdvancedPanel = html.find('#apply-damage-advanced')
    if (this.isSimpleDialog) {
      applyDamageAdvancedPanel.addClass('invisible')
      $('#apply-damage-dialog').css({ 'height': '125', 'width': '800' })
    }
    else {
      // else restore visibility and size.
      applyDamageAdvancedPanel.removeClass('invisible')
    ...
    html.find('#apply-to').find(`[value='${this.applyTo}']`)[0].selected = true

    // when dropdown changes, update this.applyTo and refresh GUI
    html.find('#apply-to').on('change', function () {
  ...
  updateUI(html) {
    ...
    if (this.applyDivisor && this.armorDivisor && this.armorDivisor !== 0) {
      ...
      html.find('#divisor-footnote').click(this._onClickPdf);
    }
  }
