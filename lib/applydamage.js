...
export default class ApplyDamageDialog extends Application {
  ...
  _locationWoundModifierCheckbox(html) { return html.find('#tactical-locationmodifier') }
  ...
  _updateModelFromInputText(element, property, converter, html) {
  ...
  _updateModelFromRadioValue(element, property, html, converter = (value) => { return value }) {
  ...
  activateListeners(html) {
    ...
    this._basicDamageInput(html).on('change', ev =>
      this._updateModelFromInputText($(ev.currentTarget), "basicDamage", this._parseIntFrom, html))
    ...
    this._userEnteredDRInput(html).on('change', ev =>
      this._updateModelFromInputText($(ev.currentTarget), "userEnteredDR", this._parseIntFrom, html))
    ...
    this._allDamageTypeRadioButtons(html).click(ev =>
      this._updateModelFromRadioValue($(ev.currentTarget), 'damageType', html))

    this._userEnteredWoundModifierInput(html).on('change', ev =>
      this._updateModelFromInputText($(ev.currentTarget), "userEnteredWoundModifier", this._parseFloatFrom, html))
    ...
    this._additionalWoundModifierInput(html).on('change', ev =>
      this._updateModelFromInputText($(ev.currentTarget), "additionalWoundModifier", this._parseFloatFrom, html))
    ...
    this._isVulnerableCheckbox(html).click(ev =>
      this._updateModelFromBooleanElement($(ev.currentTarget), 'isVulnerable', html))

    this._allVulnerabilityRadios(html).click(ev =>
      this._updateModelFromRadioValue($(ev.currentTarget), "vulnerabilityMultiple", html, parseFloat))

    this._isHardenedCheckbox(html).click(ev =>
      this._updateModelFromBooleanElement($(ev.currentTarget), 'isHardenedDR', html))

    this._allHardenedRadios(html).click(ev =>
      this._updateModelFromRadioValue($(ev.currentTarget), "hardenedDRLevel", html, parseFloat))

    html.find('#injury-tolerance').click(ev =>
      this._updateModelFromBooleanElement($(ev.currentTarget), 'isInjuryTolerance', html))

    html.find('input[type="radio"][name="injury-tolerance"]').click(ev =>
      this._updateModelFromRadioValue($(ev.currentTarget), "injuryToleranceType", html))
...
<label name='flexible-armor' for='flexible-armor'>Flexible Armor <span style='font-size: smaller;'>(Blunt
        Trauma):</span></label>
<div id="blunt-trauma-field" name="flexible-armor" class="with-button" style="margin-left: 5px;">
    <input type='text' id='blunt-trauma' class='digits-only {{classText}}' value='{{effectiveBluntTrauma}}' {{#if
        isEnabled}}{{else}}disabled{{/if}}>
    <button name='clear'><span class='fas fa-times-circle'></button>
</div>
`
    this._renderTemplate('dialog-flexible-armor-checkbox.html', {
      classText: !!this._calculator.bluntTrauma ? 'user-entered' : '',
  ...
  insertArmorDivisor(html, insertPoint) {
    ...
    `
<div name='result-divisor'>DR W/DIVISOR</div>
<div name='result-divisor'>{{effectiveDR}}</div>
<div name='result-divisor'><span>= {{DR}} &divide; {{armorDivisor}}{{#if showFootnote}}* <span id="divisor-footnote"
    class="pdflink">B379</span>{{/if}}</span></div>
`
