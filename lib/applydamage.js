...
export default class ApplyDamageDialog extends Application {
  ...
  _updateModelFromInputElement(element, property, html) {
  ...
  activateListeners(html) {
    ...
    this._basicDamageInput(html).on('change', ev =>
      this._updateModelFromInputElement($(ev.currentTarget), "basicDamage", html))
    ...
    this._userEnteredDRInput(html).on('change', ev => {
      ...
      this._calculator.userEnteredDR = this._parseIntFrom(element.val())
      // removes leading zeros or replaces blank with zero
      if (element.val() !== this._calculator.userEnteredDR.toString())
        element.val(this._calculator.userEnteredDR)
      this.updateUI(html)
    })
