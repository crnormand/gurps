...
export default class ApplyDamageDialog extends Application {
   ...
   */
  updateUI(html) {
    // turn on the correct 'tab' with Damage Type and Wounding Modifiers
    let viewName = this._calculator.woundModifierTableName
    let currentView = html.find('#wound-modifier-table>div.active')
    if (!currentView.hasClass(viewName)) {
      $(currentView).find('input[name="woundmodifier"]').attr('name', '_woundmodifier')
      html.find('#wound-modifier-table>div').removeClass('active')
      html.find(`#wound-modifier-table .${viewName}`).addClass('active')
      html.find(`#wound-modifier-table .${viewName} input`).attr('name', 'woundmodifier')
    }
    ...
    if (this._calculator.isRangedHalfDamage) {
      ...
      renderTemplate('systems/gurps/templates/damage/apply-damage-result-ranged12d.html', this._calculator).then(html => {
    ...
    if (this._calculator.isInjuryReducedByLocation) {
      ...
      renderTemplate('systems/gurps/templates/damage/apply-damage-result-crippled.html', this._calculator).then(html => {
  ...
  async _renderTemplate(template, data) {
    return renderTemplate('systems/gurps/templates/' + template, data)
  }
  ...
  async _handleEffectButtonClick(ev) {
    ...
    if (object.type === 'shock') {
      message = await this._renderTemplate('damage/effect-chat-shock.html',
        {
    ...
    if (object.type === 'majorwound') {
      message = await this._renderTemplate('damage/effect-chat-majorwound.html',
        {
          name: this.actor.data.name,
    ...
    if (object.type === 'headvitalshit') {
      message = await this._renderTemplate('damage/effect-chat-headvitalshit.html',
        {
          name: this.actor.data.name,
    ...
    if (object.type === 'knockback') {
      message = await this._renderTemplate('damage/effect-chat-knockback.html',
        {
          name: this.actor.data.name,
          yards: object.amount,
  ...
  async _populateResultEffects(html, insertPoint) {
    if (this._calculator.effects.length > 0) {
      for (let effect of this._calculator.effects) {
        let element = await renderTemplate(`systems/gurps/templates/damage/apply-damage-effect-${effect.type}.html`, effect)
...
export class DamageCalculator {
  ...
  get woundModifierTableName() {
    if (this._useLocationModifiers) {
      if (this._hitLocation === 'Vitals') return 'vitals-wound-modifiers'
      if (['Skull', 'Eye'].includes(this._hitLocation)) return 'skull-eye-wound-modifiers'
      if (this._hitLocation === 'Face') return 'face-wound-modifiers'
      if (this._hitLocation === 'Neck') return 'neck-wound-modifiers'
      if ([...extremities, ...limbs].includes(this._hitLocation)) return 'extremity-wound-modifiers'
    }
    return 'default-wound-modifiers'
  }
  ...
  get woundingModifier() {
    ...
    if (this._useLocationModifiers) {
      ...
      switch (this._hitLocation) {
        case 'Vitals':
          return this.vitalsWoundModifiers[this._damageType].multiplier

        case 'Skull':
        case 'Eye':
          return this.skullEyeWoundModifiers[this._damageType].multiplier

        case 'Face':
          return this.faceWoundModifiers[this._damageType].multiplier

        case 'Neck':
          return this.neckWoundModifiers[this._damageType].multiplier
        ...
        case 'Hand':
        case 'Foot':
          return this.extremityWoundModifiers[this._damageType].multiplier
      }
    }
  ...
  get skullEyeWoundModifiers() {
    ...
    // update the ones that need it 
    Object.keys(results)
      .filter(key => key !== 'tox' && key !== 'dmg') // everything EXCEPT toxic
      .forEach(key => results[key].multiplier = 4)
    return results
  }
  ...
  get extremityWoundModifiers() {
    ...
    Object.keys(results)
      ...
      .forEach(key => results[key].multiplier = 1)
    return results
  }
   ...
   */
  get injury() { return Math.floor(this.penetratingDamage * this.totalWoundingModifier) }
  ...
  get pointsToApply() {
    ...
    if (crippleableLocations.includes(this._hitLocation) && this._hitLocation !== 'Eye') {
  ...
  get unmodifiedPointsToApply() {
    ...
    let pointsToApply = (injury === 0) ? this._isFlexibleArmor ? this.effectiveBluntTrauma : 0 : injury
    return pointsToApply
  }
  ...
  get isCripplingInjury() {
    if (this._useLocationModifiers && crippleableLocations.includes(this._hitLocation)) {
  ...
  get isInjuryReducedByLocation() {
    return (this.isCripplingInjury && this._hitLocation !== 'Eye')
  }
  ...
  get effects() {
    ...
    if (this.injury === 0 && this._isFlexibleArmor) {
