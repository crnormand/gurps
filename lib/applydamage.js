...
export default class ApplyDamageDialog extends Application {
  ...
  */
  ...
  activateListeners(html) {
    ...
    html.find("#damage").inputFilter(function (value) {
    ...
    html.find('input[name="hitlocation"]').click(function (ev) {
      if ($(this).is(':checked')) {
        ...
        $('#result-hitlocation').text(location)
        let dr = self._getHitLocationDR(location)
        $('#result-dr').text(dr)
      }
    })

    // When "Enter DR" text changes, save the (numeric) value in this object and
    ...
    html.find('#select-dr').on('change paste keyup', function () {
      ...
      // if current hitlocation is 'Entered DR', update the result field
      let location = html.find(`input[name='hitlocation']:checked`)
      if (location.val() === 'Entered DR')
        $('#result-dr').text(self.enteredDR)
    })


    // ==== Type and Wounding Modifiers ====
    // Restrict "Enter Modifier" text to a decimal value.
    html.find("#woundmodifier").inputFilter(function (value) {
      return digitsAndDecimalOnly.test(value)
    })

    // Restrict "Additional Modifier" text to a decimal value.
    html.find("#addmodifier").inputFilter(function (value) {
      return digitsAndDecimalOnly.test(value)
    })
    ...
    html.find('input[name="woundmodifier"]').click(function (ev) {
      if ($(this).is(':checked')) {
        ...
        let modifier = GURPS.woundModifiers[value].multiplier
        $('#result-modifier').text(modifier)
      }
    })
