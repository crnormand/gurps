...
export default class ApplyDamageDialog extends Application {
  ...
  set location(text) {
    ...
    this.injury = Math.floor(this.penetrating * (this.selectedMod + this.additionalMods))
  }
  ...
  getData() {
    ...
    data.injury = Math.floor(data.penetrating * data.woundModifiers)
  ...
  activateListeners(html) {
    ...
    // ==== Results ====
    html.find('#apply-injury-publicly').on('click', ev => this._submitInjuryApply(ev, true))
    html.find('#apply-injury-secretly').on('click', ev => this._submitInjuryApply(ev, false))
  ...
  _updateUI(html) {
    ...
    html.find('[name="result-totalmod"]').text(total)
  ...
  _submitDirectApply(ev, publicly) {
    ...
    this._resolveInjury(injury, type, publicly)
  }

  _submitInjuryApply(ev, publicly) {
  ...
  _resolveInjury(injury, type, publicly) {
    let current = type === 'FP'
