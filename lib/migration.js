...
export class Migration {
  constructor() {
    ...
    this.migrations = [
      ...
      this.migrateActorComponentsToItems.bind(this), // migrate system.ads, system.skills, system.spells, system.eqt, system.melee, and system.ranged to Items
    ]
  }
  ...
  async migrateActorComponentsToItems() {
    ...
    console.log(
      'Starting migration: Convert system.ads, system.skills, system.spells, system.eqt, system.melee, and system.ranged to Items'
    )
    ...
    for (const actor of game.actors.contents) {
      ...
      // Check if migration is needed for ranged
      const hasRanged = actor.system.ranged && Object.keys(actor.system.ranged).length > 0
      const hasRangedItems = actor.items.contents.some(item => item.type === 'ranged')

      // Migrate ranged if it exists and no ranged Items yet
      if (hasRanged && !hasRangedItems) {
        const processRanged = (ranged, parentUuid = '') => {
          if (!ranged.uuid) {
            ranged.uuid = foundry.utils.randomID(16)
          }
          if (parentUuid) {
            ranged.parentuuid = parentUuid
          }
          ...
          // Process children in contains
          if (ranged.contains && Object.keys(ranged.contains).length > 0) {
            for (const childKey in ranged.contains) {
              processRanged(ranged.contains[childKey], ranged.uuid)
            }
          }
        }

        // Process all top-level ranged
        for (const rangedKey in actor.system.ranged) {
          const ranged = foundry.utils.duplicate(actor.system.ranged[rangedKey])
          processRanged(ranged)
        }
      }
    ...
    console.log(
      `Migration complete: ${migratedAdvantages} advantages, ${migratedSkills} skills, ${migratedSpells} spells, ${migratedEquipment} equipment, ${migratedMelee} melee, and ${migratedRanged} ranged migrated across ${actorsProcessed} actors`
    )
  }
}
