...
export class Migration {
      ...
      for (const tokendoc of scene.tokens.contents) {
        ...
        if (!!actor)
          ...
          for (const effect of actor.temporaryEffects) {
            ...
            let icon = orig.replace('.jpg', '.webp')
            icon = icon.replace('.png', '.webp')
  ...
  static async migrateToManeuvers(quiet) {
    ...
    for (const scene of game.scenes.contents) {
      for (const tokendoc of scene.tokens.contents) {
        ...
        // data protect against bad tokens
        if (!!actor) {
          let maneuverText = actor.data.conditions.maneuver
        }
      }
    }
  ...
  // static async fixMigration(quiet) {
  //   let v = Settings.VERSION_0104.toString()
  //   if (!quiet) ui.notifications.info('Please wait, migrating Tokens to v' + v)
  //   console.log('Migrating Tokens to v' + v)
  //   // for every scene
  //   for (const scene of game.scenes.contents) {
  //     // for every token in scene
  //     for (const tokendoc of scene.tokens.contents) {
  //       let actor = tokendoc.actor
  //       if (!!actor) {
  //         // data protect against bad tokens
  //         // for every effect on the token's actor
  //         for (const effect of actor.temporaryEffects) {
  //           let orig = effect.data.icon
  //           if (orig.startsWith('systems/gurps/icons/maneuvers/')) {
  //             let icon = orig.replace('.webp', '.png')
  //             console.log(actor.name + ':' + effect.data.label + ' ' + orig + ' -> ' + icon)
  //             let id = effect.data.flags['core']['statusId']
  //             actor.getActiveTokens()[0].toggleEffect(ManeuverData[id])
  //             await actor.deleteEmbeddedDocuments('ActiveEffect', [{ _id: effect.id }])
  //             // await actor.updateEmbeddedDocuments('ActiveEffect', [{ _id: effect.id, icon: icon }])
  //           } else if (orig.startsWith('systems/gurps/icons/')) {
  //             let icon = orig.replace('.jpg', '.webp')
  //             icon = icon.replace('.png', '.webp')
  //             console.log(actor.name + ':' + effect.data.label + ' ' + orig + ' -> ' + icon)
  //             await actor.updateEmbeddedDocuments('ActiveEffect', [{ _id: effect.id, icon: icon }])
  ...
  //         for (const e of actor.getActiveTokens()[0].data.effects) {
  //           console.log(e)
  //           actor.getActiveTokens()[0].toggleEffect(e)
  //         }
  //       }
  //     }
  //   }
