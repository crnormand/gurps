...
export class DamageCalculator {
    ...
    get penaltyToHTRollForStunning() {
        ...
        if (this.isMajorWound && ['Eye', 'Skull'].includes(this._hitLocation))
            return 10
    ...
    get isWoundModifierAdjustedForLocation() {
        if (this._useLocationModifiers) {
            // Homogenous and Diffuse: Ignore all wounding modifiers for hit location.
            if (this._isInjuryTolerance
                && (this._injuryToleranceType === DIFFUSE || this._injuryToleranceType === HOMOGENOUS))
                return false
