...
async function executeOTF(string, priv = false, event = null) {
  ...
  if (!!action.action) {
    ...
    let result = await _GURPS().performAction(action.action, _GURPS().LastActor, event)
    answer = !!result
...
async function performAction(action, actor, event = null, targets = []) {
  ...
  let _data = asGurpsActor(actor)?.getGurpsActorData()
  let prefix = ''
  let thing = ''
  ...
  if (action.type === 'modifier') {
    ...
    while (!!act && act.type === 'modifier' && act.mod) {
      ...
      _GURPS().ModifierBucket.addModifier(mod, act.desc || '')
      act = act.next
    }
    return true
  }
  ...
  if (action.type === 'roll') {
    ...
    // FIXME
    // @ts-ignore -- ModifierBucket.addModifier returns "undefined" -- so nothing is being pushed onto the array.
    if (!!action.costs) targetmods.push(_GURPS().ModifierBucket.addModifier(0, action.costs))
  }
  ...
  if (action.type === 'deriveddamage' && action.derivedformula && action.damagetype && action.formula)
    if (!!actor) {
      let df = action.derivedformula.match(/sw/i) ? _data.swing : _data.thrust
      if (!df) {
  ...
  if (action.type === 'derivedroll' && action.derivedformula)
    if (!!actor) {
      let df = action.derivedformula.match(/[Ss][Ww]/) ? _data.swing : _data.thrust
      ...
      if (tempAction.type == 'attribute') {
        ...
        if (!t && !!actor) {
          if (!!tempAction.melee) {
            ...
            let m = _GURPS().findAttack(_data, tempAction.melee)
            if (!!m) {
              th += ' for ' + m.name
            ...
            // @ts-ignore
            t = parseInt(_GURPS().resolve(tempAction.path, _data))
        ...
        // @ts-ignore
        else skill = _GURPS().findSkillSpell(_data, tempAction.name, !!tempAction.isSkillOnly, !!tempAction.isSpellOnly)
        if (!skill) {
          ...
          if (!!tempAction.floatingAttribute)
            if (!!actor) {
              ...
              // @ts-ignore
              let value = _GURPS().resolve(tempAction.floatingAttribute, _data)
              getLevel = skill => {
  ...
  if (action.type === 'skill-spell' || action.type === 'attribute') {
    ...
    if (!!bestAction.costs) _GURPS().ModifierBucket.addModifier(0, action.costs || '')
    if (!!bestAction.mod) _GURPS().ModifierBucket.addModifier(bestAction.mod, bestAction.desc || '', targetmods)
  ...
  if (action.type === 'attack' && action.name)
    if (!!actor) {
      let att = null
      prefix = ''
      att = _GURPS().findAttack(_data, action.name, !!action.isMelee, !!action.isRanged) // find attack possibly using wildcards
      if (!att) {
        if (!action.calcOnly)
          _ui().notifications?.warn(
      ...
      if (!!t) {
        ...
        // Can't roll against a non-integer
          ...
          a.shift()
          let m = a.join(' ')
          if (!!m) _GURPS().ModifierBucket.addModifier(0, m) //  Level may have "*Costs xFP"
        }
      }
      ...
      formula = '3d6'
      if (!!action.costs) _GURPS().ModifierBucket.addModifier(0, action.costs)
      if (!!action.mod) _GURPS().ModifierBucket.addModifier(action.mod, action.desc || '', targetmods)
...
async function handleRoll(event, actor, targets) {
  ...
  if ('damage' in element.dataset) {
    ...
    prefix = 'Roll vs '
    thing = _GURPS()._mapAttributePath(element.dataset.path)
    formula = '3d6'
    ...
    if (!!t) {
      ...
      // Can't roll against a non-integer
        ...
        a.shift()
        let m = a.join(' ')
        if (!!m) _GURPS().ModifierBucket.addModifier(0, m)
      }
    }
...
async function applyModifierDesc(actor, desc) {
  ...
  if (!!m && !!actor && !(actor instanceof User)) {
...
function put(obj, value, index = -1) {
  if (index == -1) {
    index = 0
    while (obj.hasOwnProperty(_GURPS().genkey(index))) index++
  }
  let k = _GURPS().genkey(index)
  obj[k] = value
  return k
}
_GURPS().put = put
 ...
 * @param {GurpsActor} record
 * @param {string} path
 */
async function removeKey(record, path) {
  ...
  let object = _GURPS().decode(record.data, objpath)
  let t = parentpath + '.-=' + objkey
  let oldRender = record.ignoreRender
  record.ignoreRender = true
  await record.update({ [t]: null }) // Delete the whole object
  delete object[key]
  i = parseInt(key)

  i = i + 1
  while (object.hasOwnProperty(_GURPS().genkey(i))) {
    let k = _GURPS().genkey(i)
    object[key] = object[k]
    delete object[k]
    key = k
    i++
  ...
  record.ignoreRender = oldRender
  await record.update({ [objpath]: sorted }, { diff: false })
}
...
async function insertBeforeKey(actor, path, newobj) {
  ...
  i = start + 1
  while (object.hasOwnProperty(_GURPS().genkey(i))) i++
  i = i - 1
...
_GURPS().sendOtfMessage = function (content, blindroll, users = null) {
...
Hooks.once('init', async function () {
  ...
  _GURPS().ModifierBucket.render(true)
...
` + cmd
    ...
    // @ts-ignore
    _user().assignHotbarMacro(macro, slot)
    return false
  })
  ...
  Hooks.once('dragRuler.ready', (/** @type {any} */ SpeedProvider) => {
    class GURPSSpeedProvider extends SpeedProvider {
       ...
       */
      getRanges(token) {
        const baseMove = asGurpsActor(token.actor).data.data.currentmove
        ...
        if (!!asGurpsActor(token.actor).data.data.additionalresources.showflightmove)
          ranges.push({ range: asGurpsActor(token.actor).data.data.currentflight, color: 'fly' })
        return ranges
      }
    }
    // @ts-ignore
