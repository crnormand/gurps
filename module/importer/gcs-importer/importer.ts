...
Portrait will not be imported.`
  ...
  async #importHitLocations() {
    ...
    this.input.settings.body_type.locations.forEach(location => {
      ...
      const tempEntry = hitlocationDictionary![this.output.additionalresources!.bodyplan!.toLowerCase()] as any
      const entry = Object.values(tempEntry).find((entry: any) => entry.id === location.id) as any
