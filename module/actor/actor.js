...
export class GurpsActor extends Actor {
	...
	async importTraitsFromGCSv2(p, cd, md) {
		...
		ts.createdon = new Date(cd).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" }).replace(" at", ",") || ''
		ts.modifiedon = new Date(md).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" }).replace(" at", ",") || ''
		// ts.modifiedon = md || ''
	...
	async importFromGCSv2(json, importname, importpath, suppressMessage = false, GCAVersion, GCSVersion) {
		...
		try {
			...
			commit = { ...commit, ...(await this.importProtectionFromGCSv2(r.settings.body_type || r.settings.hit_locations)) }
			commit = {
				...commit,
