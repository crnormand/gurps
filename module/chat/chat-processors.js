...
class WhisperChatProcessor extends ChatProcessor {
	...
	process(line) {
		...
		for (const token of destTokens) {
			let owners = game.users.contents.filter(u => token.actor.getUserLevel(u) >= CONST.DOCUMENT_PERMISSION_LEVELS.OWNER)
			...
			for (const token of canvas.tokens.placeables) {
				...
				if (!skip) {
					switch (orig.toLowerCase()) {
						case 'hp':
							output.push({ value: actor.system.HP.value, text: `${actor.name}: ${actor.system.HP.value} / ${actor.system.HP.max}`, name: actor.name })
							continue;
						case 'fp':
							output.push({ value: actor.system.FP.value, text: `${actor.name}: ${actor.system.FP.value} / ${actor.system.FP.max}`, name: actor.name })
							continue;
						case 'move':
							output.push({ value: actor.system.currentmove, text: `${actor.name}: ${actor.system.basicmove.value} / ${actor.system.currentmove}`, name: actor.name })
							continue;
						case 'speed':
							output.push({ value: actor.system.basicspeed.value, text: `${actor.name}: ${actor.system.basicspeed.value}`, name: actor.name })
							continue;
						case 'fright':
							arg = 'frightcheck'
							break
