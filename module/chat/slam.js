...
class SlamCalculatorForm extends FormApplication {
  ...
  constructor(attacker, target, options = {}) {
    ...
    this._attackerHp = !!attacker ? attacker.actor.getGurpsActorData().HP.max : 10
    this._attackerSpeed = !!attacker ? parseInt(attacker.actor.getGurpsActorData().basicmove.value) : 5

    this._targetHp = !!target ? target.actor.getGurpsActorData().HP.max : 10
    this._targetSpeed = 0
