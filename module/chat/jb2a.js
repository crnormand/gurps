...
import { wait, makeRegexPatternFrom } from '../../lib/utilities.js'
...
export default class JB2AChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    let otigons = game.packs.contents.filter(c => c.metadata.module?.startsWith('otigons'))
    if (otigons.length == 0) {
    ...
    for (const c of otigons) {
      let found = c.index.find(e => {
        console.log(e.name)
        return e.name.match(pat)
      })
      if (found) {
        c.getEntity(found._id).then(async m => {
          if (!!m) {
            await wait(t)
            m.execute()
          } else ui.notifications.warn(`Entry found for '${this.match[2]}', but no macro`)
        })
        return
      }
    }
