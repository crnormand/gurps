...
export class ItemImporter {
  ...
  _getItemCost(i) {
    if (!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_IMPORT_EXTENDED_VALUES_GCS)) {
      return !!i.calc.value ? parseFloat(i.calc.value) || 0 : parseFloat(i.value) || 0
    }

    let value
    if (!!i.calc?.extended_value) value = parseFloat(i.calc.extended_value)
    if (!value) value = !!i.calc.value ? parseFloat(i.calc.value) || 0 : parseFloat(i.value) || 0
    return value
  }

  _getItemWeight(i) {
    if (!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_IMPORT_EXTENDED_VALUES_GCS)) {
      return !!i.calc.weight ? parseFloat(i.calc.weight) || 0 : parseFloat(i.weight) || 0
    }

    let weight
    if (!!i.calc?.extended_weight) weight = parseFloat(i.calc.extended_weight)
    if (!weight) weight = !!i.calc.weight ? parseFloat(i.calc.weight) || 0 : parseFloat(i.weight) || 0
    return weight
  }
