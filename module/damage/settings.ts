...
class DamageSettingsApplication extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  static override DEFAULT_OPTIONS = {
    ...
    form: {
      closeOnSubmit: true,
      handler: DamageSettingsApplication.update,
    },
    id: 'gga-settings',
    position: {
      width: 600,
  ...
  override get title() {
    return game.i18n!.localize(SETTINGS)
  }
  ...
  protected override async _prepareContext(
    ...
    const settings =
      (Array.from(game.settings!.settings.values()).filter((s: any) => s.id.startsWith('gurps.damage')) as any) || []
    ...
    data
      ...
      .forEach(async (key: any) => {
        ...
        if (namespace !== GURPS.SYSTEM_NAME) {
          console.warn(`GURPS | DamageSettingsApplication.update: Skipping setting ${key} with namespace ${namespace}`)
          return
        }
