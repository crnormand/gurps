...
export class StatusEffect {
  static useActiveEffects = true // TODO set based on system setting
  static merged = false

  static _merge() {
    for (const key in statusEffects) {
      foundry.utils.mergeObject(statusEffects[key], getActiveEffectsData(key))
    }
    StatusEffect.merged = true
  }

  static effects() {
    if (StatusEffect.useActiveEffects && !StatusEffect.merged) {
      StatusEffect._merge()
    }
  ...
  static getAllPostures() {
    if (StatusEffect.useActiveEffects && !StatusEffect.merged) {
      StatusEffect._merge()
    }

    let postures = Object.keys(statusEffects).reduce((accumulator, key) => {
      if (foundry.utils.getProperty(statusEffects[key], 'flags.gurps.effect.type') == 'posture')
        accumulator[key] = statusEffects[key]
      return accumulator
    }, {})
    return postures
  }
...
const getActiveEffectsData = function (id) {
  const activeEffectsData = {
    ...
    prone: {
      changes: [
        ...
        {
          // TODO put this value as a chat message '{token} is [{value}]' - example: 'Joe is [-2 to be hit with ranged combat (target is Prone)]'
          key: 'chat',
