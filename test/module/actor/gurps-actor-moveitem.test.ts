...
// Helper functions
function setEquipmentQuantity(actor: GurpsActorV2<ActorType.Character>, value: number) {
  vi.spyOn(actor as any, 'promptEquipmentQuantity').mockResolvedValue(value)
}

function getUpdates(actor: GurpsActorV2<ActorType.Character>) {
  const calls = (actor.updateEmbeddedDocuments as Mock).mock.calls[0]
  const updates = calls[1] as any[]
...
function setDropPosition(actor: GurpsActorV2<ActorType.Character>, value: string | null) {
