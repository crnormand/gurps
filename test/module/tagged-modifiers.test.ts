...
import {getTagsForRoll, getRollTypeFromData, ROLL_TYPE, taggedModToApply} from '@module/tagged-modifiers/tagged-modifiers.js'
...
describe('getRollTypeFromData', () => {
    ...
    test('Tests for melee attack if no chatthing is provided', () => {
      const result = getRollTypeFromData('', { isOfType: (type: ActionType) => type === ActionType.MeleeAttack } as unknown as MeleeAttackModel)

      expect(result).toBe(ROLL_TYPE.MELEE)
    }) 

    test('Tests for ranged attack if no chatthing is provided', () => {
      const result = getRollTypeFromData('', { isOfType: (type: ActionType) => type === ActionType.RangedAttack } as unknown as RangedAttackModel)

      expect(result).toBe(ROLL_TYPE.RANGED)
    }) 
})

vi.stubGlobal('game', { i18n: {localize: (key: string) => key === 'GURPS.modifiers_.moveAndAttackRangedBulk' ? 'for bulk' : ''} })
...
describe('taggedModToApply', () => {
    ...
    ('selects appropriate modifiers based on roll type', ([chatThing, expected]) => {
        const allMods = [
            '+1 to hit in melee #melee', 
            '+2 to hit in ranged #ranged', 
            '+3 to hit #hit',
            '+4 to IQ rolls #iq',
        ]
