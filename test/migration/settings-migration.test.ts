...
const globalMock = globalThis as typeof globalThis & {
  game: {
    settings?: {
      ...
      set: jest.MockedFunction<(namespace: string, key: string, value: unknown) => Promise<void>>
    }
  }
  GURPS: any
}
...
describe('migrateLegacySettings', () => {
  ...
  beforeEach(() => {
    ...
    globalMock.game.settings = {
      ...
      set: jest.fn<(namespace: string, key: string, value: unknown) => Promise<void>>().mockResolvedValue(undefined),
    }
  })
  ...
  describe('edge cases', () => {
    it('returns early when storage is unavailable', async () => {
      globalMock.game.settings = {
        ...
        set: jest.fn<(namespace: string, key: string, value: unknown) => Promise<void>>(),
      }
